...
- pair 的单一 lifecycle lock 同时拥有 lock、master-open 与 slave-open count。master→slave raw input 进入同一个 Terminal line discipline；slave→master output 使用独立 byte Pipe，小块原子写与真实 write-capacity wait 保证 CRLF 转换不会部分提交，阻塞/nonblocking write 不会返回伪零进度。两端另有不进入字节流的 readiness Pipe，slave `POLLOUT` 直接复查 byte Pipe 容量，因此 input/output、hangup、poll/epoll generation 不会靠伪字节同步。slave 最后关闭使 master read 返回 `EIO/POLLHUP`；master 最后关闭使 slave read 返回 EOF、write `EIO/POLLHUP`，并经 composition-root 注入的 task seam 对 foreground group 发布 SIGHUP/SIGCONT。
