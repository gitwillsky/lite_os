...
| Unix domain socket | socket syscall family | Phase 46 abstract namespace complete | 与 Pipe 共用 OFD/readiness/wait；pathname namespace仍需 VFS socket inode |
| 等待多 fd | `ppoll` 73 / epoll 20-22 | Partial | Phase 46/47 收敛到同一 readiness generation 与 indexed Poll membership；epoll 尚无 `EPOLLEXCLUSIVE` wake-one |
