...
3. **动态装载与设备扩展**：分别设计 `PT_TLS/ET_DYN/PT_INTERP` 和标准 fd/ioctl/mmap/poll UAPI，不与当前静态路径并行维护半实现。
