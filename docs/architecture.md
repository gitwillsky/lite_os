...
- vDSO 与 `AT_HWCAP`。
- TCP、IPv6、raw/ICMP userspace socket、DHCP、多 NIC/network namespace、GPU/input user device ABI，以及完整 VMIN/VTIME。
...
3. **网络 I/O**：Phase 49 已在同一 socket/OFD seam 下完成 VirtIO-net、Ethernet/ARP/IPv4 control、AF_INET UDP、standard interface/route ioctl、sendmsg/recvmsg ancillary data 与 procfs network projection；下一竖切在该唯一 stack owner 上接入 TCP，raw/ICMP userspace socket 单独按 Linux 语义开放。
