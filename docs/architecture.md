...
U-mode 使用 Linux/riscv64 约定：`a7=number`、`a0..a5=args`、`a0=result`、kernel error 为负 errno。共享 crate 只有 35 个已实现的 Linux number，未识别 number 一律 `-ENOSYS`。
...
- signal、futex timeout/PI/requeue 与完整 pthread primitive。
...
1. **signal 竖切**：`rt_sigaction/rt_sigprocmask/rt_sigreturn/tgkill`、riscv64 rt frame、pending selection 和 syscall interruption 一次成立。
2. **musl 验收竖切**：补齐当前明确列出的 VMA/futex/clone 缺口后固定一个 musl commit，以静态非线程程序开始，再扩展 pthread/dynamic linker。
