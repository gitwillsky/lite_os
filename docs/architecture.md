...
- `MemorySet` 的有序 VMA 表唯一拥有 ELF LOAD、`brk` heap、256 KiB RW+NX stack、anonymous/file private mapping、anonymous/file shared mapping、supervisor-only TrapContext 与 kernel mapping；不存在独立 heap/mmap shadow registry。
...
- anonymous private/shared `mmap` eager 分配清零页，file private mmap 从 inode 建立私有帧；anonymous shared backing 以唯一 Arc 跨 fork 保持 frame identity，private mapping 继续 COW；`munmap/mprotect` 拆分 VMA，destructive `MAP_FIXED` 只替换 mmap-owned 区间。提交前验证完整区间，映射 OOM 回滚页表和 frame owner。

当前实现 anonymous/file private eager mapping、anonymous `MAP_SHARED` eager mapping，以及 regular-file `MAP_SHARED` lazy mapping，支持 `PROT_NONE`、地址 hint、`MAP_FIXED` 与 `MAP_FIXED_NOREPLACE`。`fs::page_cache` 是 regular-file 数据页唯一 owner：read/write、ELF source、shared fault、msync/fsync/sync 与 truncate 都经同一 cache seam；ext2 只实现 storage adapter。共享 fault 按地址空间局部 VMA 装入同一物理页，fork 保持 shared backing，private 映射继续 COW。truncate 通过 memory-owned weak address-space registry 撤销 EOF 外 resident PTE，后续访问交付 SIGBUS；munmap、exit 与同步 syscall 完成 dirty writeback。物理页分配压力只同步回收无外部引用的 clean cache page，dirty page 不被无写回丢弃。当前不提供 tmpfs/shmem pathname object 或后台 writeback/reclaim worker。

## 8. Process、Thread 与生命周期

`TaskControlBlock` 显式组合：

- `Process`：Arc-owned TGID、可原子替换的 AddressSpace owner handle、cwd opened entry、FileDescriptorTable，由 thread group 共享。相对 pathname 直接从 cwd opened entry 解析；`getcwd` 从 VFS-owned parent/name/deleted 链生成 raw absolute path，不保留可能随 rename 漂移的 path cache。
...
`execve` 通过 inode-backed `ExecutableSource` 只读取固定 ELF header、至多 64 KiB program-header table 与 PT_INTERP pathname，生成唯一映射计划；`MemorySet` 再把每个 PT_LOAD 直接按页读入新 frame，BSS 使用 frame 的零初始化区域。任一 source read、frame allocation、AddressSpace owner 注册、映射或 initial stack 构造失败都会丢弃整个新映像，不修改旧 handle；成功后才一次替换 Process 的 AddressSpace，trap 返回点不用旧 syscall result 覆盖新入口。当前普通多线程 Process 的直接 exec 仍返回 `EAGAIN`；vfork child 是独立单线程 Process，可在 sibling parent Thread 继续运行时完成 detach。
