...
- 47 个 Linux/riscv64 syscall number；UART RX IRQ、termios line discipline 与 process-group-aware TTY 通过统一 Terminal OFD/wait registry 支撑真实输入，文件入口统一经过 fd/OFD 与 VFS，未实现编号返回 `-ENOSYS`。

LiteOS 当前不支持 futex PI/requeue/bitset、完整 clone/vfork flags、所有 syscall 的 restart、signal altstack/realtime queue/process-directed delivery、pipe/socket/IPC、file/shared/lazy mapping、PIE、动态链接、完整 pthread runtime、默认 shell rootfs、GUI、network userspace 或常规 musl 程序。当前只对 blocking `wait4` 和无 timeout 的 futex WAIT 实现 `SA_RESTART`；固定 pthread consumer 通过不外推为通用 musl 兼容。项目不声称完整 Linux 兼容、POSIX.1-2024 符合或 musl 兼容。
...
- BusyBox：官方 release `1.37.0`，作为最终唯一 `init + ash` userspace consumer；独立 rootfs gate 已启动 init、注入 UART 输入并执行 ash builtin，默认 `make build` 尚未切换。
...
该目标先完成 musl gate，再校验 BusyBox 官方 tarball SHA-256，应用唯一 `user/busybox.config` 与 `user/inittab`，构建 RISC-V 静态 `ET_EXEC` 和单 inode hardlink applet rootfs。gate 冷启动 BusyBox init，等待 UART `askfirst`，注入命令并要求 ash 计算/输出 `LITEOS_BUSYBOX_SHELL_42`，随后用 VINTR 中断 foreground ash loop 并继续执行；同时检查无 dynamic interpreter、无 W+X LOAD、用户栈不可执行。BusyBox 源码和二进制不进入仓库。该路径证明当前交互 tracer bullet，不表示 pipe、完整 stop/continue job control、全部 termios action 或全部 applet 已成立。
...
本仓库不维护、不修正、不执行测试用例。统一运行 `make verify`；它执行 AST 架构围栏、workspace/组件构建、Clippy、ELF 静态检查、`-smp 1/3/8` Rust init 冷启动、固定 musl pthread consumer 冷启动，以及固定 BusyBox `init + ash` UART 交互 gate。各阶段的证据保存在 `docs/phase-*.md`。
