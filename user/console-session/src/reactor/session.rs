...
pub(super) fn replay_boot_log(model: &mut Model) {
    let fd = unsafe {
        ffi::open(
...
pub(super) fn spawn_shell(columns: usize, rows: usize, mode: DrmMode) -> Option<(i32, i32)> {
    let master = unsafe {
        ffi::open(
...
pub(super) fn terminate_child(child: i32) {
    if child <= 0 {
        return;
    }
