...
- 41 个 Linux/riscv64 syscall number；UART RX IRQ 通过统一 console OFD/wait registry 支撑真实输入，文件入口统一经过 fd/OFD 与 VFS，未实现编号返回 `-ENOSYS`。

LiteOS 当前不支持 futex PI/requeue/bitset、完整 clone/vfork flags、所有 syscall 的 restart、signal altstack/queue/process-directed delivery、pipe/socket/IPC、file/shared/lazy mapping、PIE、动态链接、完整 pthread runtime、shell、GUI、network userspace 或常规 musl 程序。当前只对 blocking `wait4` 和无 timeout 的 futex WAIT 实现 `SA_RESTART`；固定 pthread consumer 通过不外推为通用 musl 兼容。项目不声称完整 Linux 兼容、POSIX.1-2024 符合或 musl 兼容。
