...
唯一强制验证入口是 `make verify`。静态围栏、build 与 artifact gate 成功后，topology、musl、BusyBox 与 APK runtime gate 才可并发；每条 gate 必须使用独占可写镜像、独立 success stamp，网络 fixture 必须使用不相交的内部 host port domain。recursive Make 等待全部 gate，任一失败都禁止进入最终 diff hygiene；禁止以并发为由跳过、抽样或提前发布成功证据。
