...
BusyBox 官方 release 1.37.0 已固定 tarball SHA-256、唯一 `init + ash`/基础 applet config 与 inittab。默认 `make build` 构造并静态检查单 BusyBox inode/hardlink rootfs，`make run` 直接启动该镜像；仓库不再存在 Rust init/runtime 或另一条 rootfs 路径。选中 applet 还包括 `ifconfig/route/nc/netstat`：QEMU user-net 下可经标准 ioctl 配置 `eth0` 与 default route，`nc -u` 通过 `recvmsg/MSG_PEEK/IP_PKTINFO` 完成双向 UDP，`nc` 的 active connect 与 passive listen/accept 完成双向 TCP stream。现有 gate 验证 ash 算术、pipeline、重定向、后台 wait、可执行 script、process-group kill、`ps/free/uptime/top/df/uname/arch/date` 系统可观测性、64 次动态 exec/exit 后的内存回收、跨冷启动持久化、foreground Ctrl-Z/jobs/bg/fg/Ctrl-C，以及 `stty` 驱动的后台 read/SIGTTIN 与 `TOSTOP` write/SIGTTOU 停止-恢复链。当前不证明未选中 applet。

## 13. Syscall ABI

U-mode 使用 Linux/riscv64 约定：`a7=number`、`a0..a5=args`、`a0=result`、kernel error 为负 errno。共享 crate 只有 103 个已实现的 Linux number，未识别 number 一律 `-ENOSYS`。
...
- vDSO 与 `AT_HWCAP`。
- IPv6、raw/ICMP userspace socket、DHCP/DNS、多 NIC/network namespace、TCP reuse/keepalive/linger policy、GPU/input user device ABI，以及完整 VMIN/VTIME。
...
3. **网络 I/O**：Phase 49 完成 VirtIO-net、Ethernet/ARP/IPv4 control 与 AF_INET UDP；Phase 50 在同一 NetworkStack/Socket/OFD seam 上完成 TCP active/passive stream、backlog、readiness、shutdown 与 close-time FIN/TIME_WAIT lifecycle。下一竖切应优先补 DNS/DHCP 与真实 TCP application consumer，raw/ICMP userspace socket 单独按 Linux 语义开放。
