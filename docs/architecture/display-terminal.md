...
- `/bin/liteui-compositor` 是唯一 DRM/evdev/netlink consumer。main reactor 拥有 Display/Scene、两只 dumb buffer、page-flip、pointer/focus/window tree 与 resize；固定 presenter pthread 只执行单槽 SPSC 中的 blocking DIRTYFB。pointer damage 只覆盖旧/新 cursor 或受影响 window，request 在途时 render/resize deadline 从 poll timeout 排除，idle 时两线程都无限阻塞。
