...
U-mode 使用 Linux/riscv64 约定：`a7=number`、`a0..a5=args`、`a0=result`、kernel error 为负 errno。共享 crate 只有 99 个已实现的 Linux number，未识别 number 一律 `-ENOSYS`。
...
- pathname AF_UNIX、AF_INET 与通用设备 fd/ioctl/mmap UAPI。
...
2. **事件与本地 I/O**：Phase 46/47 已在统一 OFD/readiness/wait seam 上完成 abstract AF_UNIX stream/datagram 与 epoll 核心语义；`EPOLLEXCLUSIVE` 仍明确不在当前 wait model 的支持范围。
3. **网络 I/O**：在同一 socket/OFD seam 下接入 VirtIO-net、ARP/IPv4/ICMP/UDP/TCP 与 AF_INET。
