...
flowchart TD
    ...
    K --> M["memory / Sv39"]
    K --> T["task / scheduler"]
    K --> D["PLIC / RTC / VirtIO block, net, RNG, GPU"]
...
7. PLIC 与 modern-only VirtIO block、RNG、GPU、network discovery；
8. composition root 将 primary display seam 与统一 Pipe notification 装配为 DRM owner，将 primary block adapter 装配为 ext2 root，再把 devfs、procfs 与只读 CPU-topology sysfs adapter 分别挂载到 `/dev`、`/proc`、`/sys`；
...
- 当前尚未发布 page-flip event/target/async flags、DRM master/auth、atomic KMS 或 input UAPI；固定 single-CRTC legacy userspace 已可通过标准 ABI 把 dumb framebuffer 切换为真实 hardware scanout。
...
- 旧私有 GUI/framebuffer、VirtIO GPU/input UAPI、Web/window manager、字体与用户演示；当前显示路径只发布标准 DRM topology 与 dumb-buffer ABI，不保留私有入口。
...
- pathname AF_UNIX 与 DRM 之外的通用设备 fd/ioctl/mmap UAPI。
...
- vDSO 与 `AT_HWCAP`。
- IPv6、非 ICMP raw IP protocol、Ethernet ARP packet protocol、多 NIC/network namespace、TCP keepalive/linger policy，以及 DRM page-flip event/atomic KMS 与 input user ABI。
...
4. **显示 I/O**：modern VirtIO-GPU 2D、single-scanout fallback、六阶段 IRQ-driven resource 切换、标准 `/dev/dri/card0` query、per-file GEM/dumb mmap、framebuffer lifecycle、SETCRTC 与无 event PAGE_FLIP 已闭环；下一竖切是 DRM event/read/poll 与 master ownership，再进入 atomic KMS/input，禁止暴露 VirtIO 私有 ioctl、CPU 全帧 memcpy 或恢复运行期自旋。
