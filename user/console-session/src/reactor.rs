...
use session::{read_pty, replay_boot_log, set_window_size, spawn_shell, terminate_child};
...
/// File descriptors that belong to one active console reactor invocation.
struct SessionDevices<'a> {
    master: i32,
    keyboard: i32,
    pointer: &'a mut Option<Pointer>,
    netlink: i32,
}
...
fn report_resize_failure(error: DisplayError) {
    let message: &[u8] = match error {
        ...
        DisplayError::OverBudget => {
            b"console-session: resize exceeds framebuffer budget; preserving active mode\n"
        }
        DisplayError::OutOfMemory => {
            b"console-session: resize out of memory; preserving active mode\n"
        }
        DisplayError::System => {
            b"console-session: resize transaction failed; preserving active mode\n"
        }
    };
    let mut written = 0;
