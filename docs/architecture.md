...
8. composition root 将 primary block adapter 装配为 ext2 root，再把 devfs、procfs 与只读 CPU-topology sysfs adapter 分别挂载到 `/dev`、`/proc`、`/sys`；
...
- vDSO 与 `AT_HWCAP`。
- IPv6、非 ICMP raw IP protocol、Ethernet ARP packet protocol、多 NIC/network namespace、TCP keepalive/linger policy，以及 GPU/input user device ABI。
