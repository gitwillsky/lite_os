...
impl DecoderSession {
    ...
    pub(super) fn open_range(path: &Path, offset: u64, length: u64) -> Result<Self, String> {
        let file = Box::new(BoundedFile::open(path, offset, length)?);
    ...
    pub(super) fn seek(&mut self, seconds: f64) -> Result<(), String> {
        if self.eof {
            ...
            // bounded source before seeking so replay-after-ended follows the
            // same accurate seek path instead of depending on demuxer EOF state.
            let path = self.path.clone();
            *self = Self::open_range(&path, self.source_offset, self.source_length)?;
        }
