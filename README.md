...
- 27 个 Linux/riscv64 syscall number；文件入口统一经过 fd/OFD 与 VFS，未实现编号统一返回 `-ENOSYS`。

LiteOS 当前不支持 fork/clone/wait、signal、futex、pipe/socket/IPC、`mmap`、PIE、动态链接、TLS、shell、GUI、network userspace 或常规 musl 程序。项目不声称完整 Linux 兼容、POSIX.1-2024 符合或 musl 兼容。
