...
use crate::{
    ...
    input::{self, InputQueue, KeyboardState, MAX_KEY_BYTES, PTY_REPLY_EXPANSION, flush_input},
    model::Model,
...
const CONNECT_RETRY_MS: u64 = 1_000;
...
pub fn run(command: &[u8]) -> i32 {
    ...
    let Some(drm) = handshake(&socket) else {
        report(b"terminal: handshake failed\n");
        return 1;
    };
    let Some(mut surface) = Surface::create(&drm, WIDTH, HEIGHT) else {
        report(b"terminal: surface failed\n");
        return 1;
    };
    // handle 所有权随本条消息转移给桌面；此后本进程绝不 DESTROY_DUMB。
    let Some(surface_id) = create_surface(&socket, surface.handle()) else {
        report(b"terminal: create failed\n");
        return 1;
    };
    surface.transfer_handle();
    if !command.is_empty() {
    ...
    let Ok(mut pty) = PtySession::spawn_shell(WindowSize {
        columns: COLUMNS as u16,
        rows: ROWS as u16,
        pixel_width: u16::try_from(WIDTH).unwrap_or(0),
        pixel_height: u16::try_from(HEIGHT).unwrap_or(0),
    }) else {
    ...
    inject_command(&mut pty, command);
...
fn connect() -> Option<UnixStream> {
    for _ in 0..CONNECT_RETRIES {
        if let Ok(stream) = UnixStream::connect(proto::SOCKET_PATH) {
            return Some(stream);
        }
...
fn handshake(socket: &UnixStream) -> Option<DrmDevice> {
    ...
    let (count, drm_fd) = match proto::recv_frame_blocking(socket, &mut buf) {
    ...
    let drm_fd = drm_fd?;
    Some(DrmDevice::from_owned_fd(drm_fd))
}

/// `CREATE_SURFACE` → `SURFACE_CREATED`，返回分配到的 surface id（`error == 0`）。
fn create_surface(socket: &UnixStream, gem_handle: GemHandle) -> Option<u32> {
    let mut buf = [0u8; proto::MAX_MESSAGE];
    let length = proto::CreateSurface::encode(&mut buf, WIDTH, HEIGHT, gem_handle.get(), 0, TITLE)?;
    proto::send_message(socket, &buf[..length]).ok()?;
    let (count, unused_fd) = proto::recv_frame_blocking(socket, &mut buf).ok()?;
    if unused_fd.is_some() {
        return None;
    }
    if count == 0 {
        return None;
...
fn send_set_title(socket: &UnixStream, surface_id: u32, title: &[u8]) {
    let mut buf = [0u8; proto::MAX_MESSAGE];
...
fn inject_command(session: &mut PtySession, command: &[u8]) {
    if command.is_empty() {
        return;
    }
    let mut line = Vec::new();
    if line
        .try_reserve_exact(command.len().saturating_add(1))
        .is_err()
    {
        return;
    }
    ...
    while written < line.len() {
        match session.write(&line[written..]) {
            Ok(0) => return,
...
fn event_loop(
    ...
    let surface_id = session.surface_id;
    ...
    let mut last_present = monotonic_milliseconds();
    ...
    loop {
        ...
        if render_due.is_some_and(|deadline| deadline <= now) {
            if let Some(damage) = render::present(surface, model, atlas, metrics, focused)
                && send_commit(&session.socket, surface_id, &damage.rects[..damage.count]).is_err()
            {
                return 1;
            }
            render_due = None;
        ...
        let timeout = timeout(render_due, blink_due, monotonic_milliseconds());
        let (pty_ready, socket_ready) = loop {
            let pty_interest = (if input.remaining() >= PTY_REPLY_EXPANSION {
                PollEvents::READ
            } else {
                PollEvents::EMPTY
            }) | if input.is_empty() {
            ...
            let mut descriptors = [
                PollFd::new(session.pty.as_fd(), pty_interest),
                PollFd::new(session.socket.as_fd(), PollEvents::READ),
            ];
            match unix::poll(&mut descriptors, timeout) {
                Ok(_) => break (descriptors[0].returned(), descriptors[1].returned()),
                Err(error) if error.kind() == io::ErrorKind::Interrupted => continue,
                Err(_) => return 1,
            }
        };
        ...
        if pty_ready.contains(PollEvents::READ | PollEvents::ERROR | PollEvents::HANGUP) {
            let (changed, ended) = read_pty(&mut session.pty, model, &mut input);
            // Device-status/attributes 查询属于 PTY request/reply；同轮立即写回可避免全屏程序
            // 等待下一次 POLLOUT edge 或超时，键盘/鼠标输入仍与回复共用唯一有界队列。
            flush_input(&mut session.pty, &mut input);
            closed = ended;
            if changed {
                schedule_render(&mut render_due, last_present, now);
        ...
        if pty_ready.contains(PollEvents::WRITE) {
            flush_input(&mut session.pty, &mut input);
        }
        if socket_ready.contains(PollEvents::READ | PollEvents::ERROR | PollEvents::HANGUP) {
            match drain_socket(&session.socket, &mut rx, &mut rx_len) {
                SocketStatus::Open => {}
...
fn drain_socket(
    ...
    loop {
        ...
        match socket.read(&mut rx[*length..]) {
            Ok(0) => return SocketStatus::Closed,
            Ok(count) => *length += count,
            Err(error) if error.kind() == io::ErrorKind::Interrupted => continue,
            Err(error) if error.kind() == io::ErrorKind::WouldBlock => return SocketStatus::Open,
            Err(_) => return SocketStatus::Fatal,
        }
    }
}
...
fn timeout(render: Option<u64>, blink: Option<u64>, now: u64) -> Option<Duration> {
