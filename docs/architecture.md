...
每个 Process 拥有唯一 `FileDescriptorTable`；fd entry 保存 `FD_CLOEXEC`，dup 后共享同一 OFD 的 offset/status flags。PID 1 的 `0/1/2` 在建进程时安装为同一个 Terminal 的 OFD，`read/write/writev/ioctl/close/dup/fcntl` 不存在 console syscall 旁路。Terminal 唯一拥有 termios、cooked input、window size、controlling session 与 foreground PGID；UART ring 只保存 hardirq raw bytes。deferred softirq 执行输入变换、canonical/echo/erase/EOF 和 ISIG，再唤醒统一 Console wait membership。
...
U-mode 使用 Linux/riscv64 约定：`a7=number`、`a0..a5=args`、`a0=result`、kernel error 为负 errno。共享 crate 只有 47 个已实现的 Linux number，未识别 number 一律 `-ENOSYS`。
