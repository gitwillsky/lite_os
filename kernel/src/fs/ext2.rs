...
impl Ext2FileSystem {
    ...
    fn alloc_from_bitmap(&self, bitmap_block: u32, total: usize) -> Result<u32, FileSystemError> {
        ...
        // Debug high-frequency bitmap operations
        static BITMAP_ALLOC_COUNT: AtomicUsize = AtomicUsize::new(0);
        let count = BITMAP_ALLOC_COUNT.fetch_add(1, Ordering::Relaxed) + 1;
        if count > 800 && count % 100 == 0 {
            warn!(
                "[EXT2_BITMAP] High allocation activity: {} bitmap allocations, block={}",
                count, bitmap_block
            );
        }

        // Retry loop with exponential backoff for concurrent access
        for retry in 0..5 {
            let mut buf = vec![0u8; self.block_size];
            self.read_block(bitmap_block, &mut buf)?;
            let max_bits = cmp::min(total, self.block_size * 8);

            // Count free bits for diagnostics (only on first attempt)
            if retry == 0 {
                let mut free_count = 0;
                for (byte_index, b) in buf.iter().enumerate().take(ceil_div(max_bits, 8)) {
                    for bit in 0..8 {
                ...
                if free_count <= 10 {
                    warn!(
                        "[EXT2_BITMAP] LOW RESOURCES: Only {} free bits in bitmap block {}",
                        free_count, bitmap_block
                    );
                }
            ...
            // Try to find and allocate a free bit
            ...
            for (byte_index, b) in buf.iter_mut().enumerate().take(ceil_div(max_bits, 8)) {
                if *b != 0xFF {
                    ...
                    for bit in 0..8 {
                        ...
                        if (*b & (1u8 << bit)) == 0 {
                            *b |= 1u8 << bit;
                            found_bit = Some(bit_index as u32);
                            break;
                        }
                    }
                    if found_bit.is_some() {
                        break;
                    }
                }
            }

            if let Some(bit_idx) = found_bit {
                // Try to write back atomically
                match self.write_block(bitmap_block, &buf) {
                    Ok(_) => return Ok(bit_idx),
                    Err(e) => {
                        if retry < 4 {
                            warn!(
                                "[EXT2_BITMAP] Write collision on retry {}, backing off...",
                                retry
                            );
                            // Simple exponential backoff - in a real system you'd use a proper delay
                            for _ in 0..(1 << retry) {
                                core::hint::spin_loop();
                            }
                            continue;
                            ...
                            error!(
                                "[EXT2_BITMAP] Failed to write bitmap after {} retries",
                                retry
                            );
                            return Err(e);
                        }
            ...
            // No free bits found in this attempt
            if retry < 4 {
                warn!(
                    "[EXT2_BITMAP] No free bits found on attempt {}, retrying...",
                    retry + 1
                );
                continue;
            }
        ...
        error!(
            "[EXT2_BITMAP] EXHAUSTED: No free bits in bitmap block {} after 5 attempts",
            bitmap_block
        );
    ...
    fn allocate_block_in_group(&self, group: usize) -> Result<u32, FileSystemError> {
        // Get bitmap block number and check free count atomically
        let (bitmap_block, has_free) = {
            let groups = self.groups.lock();
            let gd = groups
                .get(group)
                .ok_or(FileSystemError::InvalidFileSystem)?;
            (gd.bg_block_bitmap, gd.bg_free_blocks_count > 0)
        };
        ...
        // Allocate from bitmap (this handles concurrent access internally)
        let rel = self.alloc_from_bitmap(bitmap_block, self.blocks_per_group)?;

        // Update group descriptor atomically
        {
            ...
            if gd.bg_free_blocks_count == 0 {
                // Race condition: another thread exhausted the group
                // We need to free the bit we just allocated
    ...
    fn allocate_inode_in_group(&self, group: usize) -> Result<u32, FileSystemError> {
        ...
        let (bitmap_block, has_free) = {
            ...
            let gd = groups
                .get(group)
                .ok_or(FileSystemError::InvalidFileSystem)?;
            (gd.bg_inode_bitmap, gd.bg_free_inodes_count > 0)
        };
        ...
        // Allocate from bitmap (this handles concurrent access internally)
        let rel = self.alloc_from_bitmap(bitmap_block, self.inodes_per_group)?; // 0-based within group
...
impl Ext2Inode {
    ...
    fn ensure_block_mapped(&self, file_block_index: u32) -> Result<u32, FileSystemError> {
        ...
        // Thread-safe implementation to prevent deadlocks and race conditions
        ...
        loop {
            ...
            while pos < self.fs.block_size {
                ...
                if hdr.inode != 0 && spare >= needed {
                    ...
                    let mut new_hdr = Ext2DirEntry2Header {
                        inode: child_inode,
            ...
            blk_index += 1;
            if (blk_index as usize) * self.fs.block_size > 16 * 1024 * 1024 {
        ...
        for i in new_blocks..old_blocks {
            if i < 12 {
                ...
                if ind != 0 {
                    ...
                    unsafe {
                        ptr::write_unaligned((ibuf.as_mut_ptr() as *mut u32).add(idx), 0u32);
                    }
