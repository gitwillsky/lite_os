...
- 产品 userspace 是按所选架构原生构建的固定 musl runtime、BusyBox `init + ash`、dependency-free Rust `desktop`/`terminal`/`display-proto` 图形栈和单 ELF `liteos-stress` diagnostics。kernel、rootfs、APK 与 cache 都携带同一个 architecture identity。
