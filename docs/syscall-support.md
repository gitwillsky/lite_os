...
| 198-212, 242 | AF_UNIX 与 AF_INET UDP/TCP socket | socket/sockaddr/iovec/msghdr/cmsg/options | fd/count；标准 socket errno | Linux sockets；musl；BusyBox nc | **Partial（明确 domain/protocol scope）**。AF_UNIX abstract stream/datagram 保持原范围；AF_INET UDP 支持 bind/connect/sendto/recvfrom/read/write、sendmsg/recvmsg、MSG_PEEK/DONTWAIT/TRUNC、IP_PKTINFO；IPv4 TCP 支持 bind(0)、blocking/nonblocking connect、listen/accept4（backlog cap 16）、partial stream I/O、ppoll/epoll、SO_ERROR、peer EOF、shutdown 与 close-time FIN/TIME_WAIT。单 VirtIO-net interface 由标准 ioctl 配置，smoltcp 0.13.1 唯一拥有 ARP/IPv4/UDP/TCP 状态并使用 Reno。pathname AF_UNIX、IPv6、raw/ICMP、multicast、TCP urgent data、reuse/broadcast/keepalive/linger 的完整 policy 尚未开放。 | `kernel/src/syscall/socket.rs`, `kernel/src/syscall/socket/message.rs`, `kernel/src/syscall/socket/options.rs`, `kernel/src/socket.rs`, `kernel/src/socket/inet.rs`, `kernel/src/socket/inet/{udp,tcp}.rs`, `kernel/src/socket/unix.rs` |
...
当前 103 个入口支撑固定 musl pthread consumer、动态 BusyBox、`dlopen`、AF_UNIX epoll event-loop 与 AF_INET UDP/TCP consumer。验证仍不表示任意 musl 程序可运行；剩余缺口包括：
...
5. IPv6、raw/ICMP userspace socket、DHCP/DNS、multicast、多 interface/network namespace，以及 TCP advanced option/urgent-data policy。
