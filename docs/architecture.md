...
Signal disposition 与 coalesced process-directed pending bit+首个 siginfo 由共享 Process 的同一 `ProcessSignalState` lock 拥有；mask、thread-directed pending 与至多一条 syscall replay record属于 Thread。`tgkill` 发布 SI_TKILL 到 Thread；`kill` 按 TGID/caller-PGID/all-except-init-and-caller/explicit-PGID selector 发布 SI_USER 到 Process shared pending；TTY group signal 与 SIGCHLD 复用同一路径。global PID 1 丢弃默认 disposition signal，但显式 handler 与 blocked pending 仍生效；强制同步 fault 不走该豁免。stop signal generation 清除所有 pending SIGCONT；SIGCONT generation 清除所有 pending stop signal，并无视 mask/disposition 立即恢复 Process。默认 stop action 经 process graph 发起 group stop，最后一个 Thread 进入 `Stopped` 后提交 `CLD_STOPPED`；orphan group 在 delivery 时丢弃默认 SIGTSTP/SIGTTIN/SIGTTOU。continue 提交可独立消费的 `CLD_CONTINUED`。被默认 stop action 中断的 syscall 在停止前恢复原始 ecall context，SIGCONT 后确定性重放，不错误暴露 `EINTR`。Process exit/reparent 导致 stopped group 新近 orphaned 时，对冻结的成员集合依次发布 SIGHUP、SIGCONT。默认致命 signal 提交 signal wait status并终止完整 Thread Group；parent 分别获得 `CLD_EXITED` 或 `CLD_KILLED`。delivery 与 `rt_sigtimedwait` 都先消费 Thread pending、再消费 shared pending；未屏蔽、非忽略 signal 会取消 interruptible wait。trap return 构造 Linux RV64 signal frame，`rt_sigreturn` 恢复上下文。当前 blocking `wait4` 和无 timeout futex WAIT 可重启；relative-timeout wait 保持 `EINTR`。无 altstack、queued realtime value或 credential permission check。
...
- persistent root 是同步读写 ext2 revision 1；内存 device filesystem 挂载到预建的 `/dev` mountpoint，提供 `null/zero/tty/console`；只读 procfs 挂载到预建的 `/proc`，按 Linux 文本格式投影 `/proc/stat`、`meminfo`、`loadavg`、`uptime`、`mounts` 与 live `/proc/<pid>/stat`。procfs 与 `sysinfo` 共用 task façade 的一次采集边界，只投影 process、frame、scheduler、timer 与 per-hart runtime 的唯一 owner 状态；mounts 则直接投影 VFS 唯一 mount table。inode API 统一承载 metadata、目录与 mutation，打开后的 character device、pipe 与 regular inode 共用 OFD/fd table。
...
BusyBox 官方 release 1.37.0 已固定 tarball SHA-256、唯一 `init + ash`/基础 applet config 与 inittab。默认 `make build` 构造并静态检查单 BusyBox inode/hardlink rootfs，`make run` 直接启动该镜像；仓库不再存在 Rust init/runtime 或另一条 rootfs 路径。gate 验证 ash 算术、pipeline、重定向、后台 wait、可执行 script、process-group kill、`ps/free/uptime/top/df` 系统可观测性、跨冷启动持久化、foreground Ctrl-Z/jobs/bg/fg/Ctrl-C，以及 `stty` 驱动的后台 read/SIGTTIN 与 `TOSTOP` write/SIGTTOU 停止-恢复链。当前不证明未选中 applet。

## 13. Syscall ABI

U-mode 使用 Linux/riscv64 约定：`a7=number`、`a0..a5=args`、`a0=result`、kernel error 为负 errno。共享 crate 只有 65 个已实现的 Linux number，未识别 number 一律 `-ENOSYS`。
...
- vfork、完整 clone flags与跨 sibling 的 exec 事务。
- signal altstack/realtime queue、credential permission policy、其他 syscall 的 restart coverage、带 relative timeout futex 的剩余时间重启、futex PI/requeue/bitset 与完整 pthread primitive。
...
- vDSO 与 `AT_HWCAP`。
- network/GPU/input user device ABI，以及完整 VMIN/VTIME。
...
1. **系统身份可观测性**：按 Linux `uname` ABI 由唯一 platform/build identity owner 生成 utsname，并以 BusyBox `uname/arch` 形成下一条可验证竖切。
2. **完整 identity/permission 竖切**：只在同时覆盖 Process credentials、VFS traversal/inode mode、exec 与 signal permission 时引入非 root identity，禁止先铺无可观察语义的 credentials 字段。
