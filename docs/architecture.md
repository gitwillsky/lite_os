...
BusyBox 官方 release 1.37.0 已固定 tarball SHA-256、唯一 `init + ash`/基础 applet config 与 inittab。独立 gate 构造单 BusyBox inode/hardlink rootfs，冷启动 init，等待 UART `askfirst`，验证 ash 算术、外部 applet pipeline、文件重定向、后台 subshell+wait 与 foreground Ctrl-C。第一次 1-hart 启动写入并 `sync` 持久化文件，第二次 8-hart 启动复用同一镜像读回。默认 `make build` 尚未切换，且当前不证明 stop/continue job control、全部 termios action 或未选中 applet。
