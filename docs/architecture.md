...
U-mode 使用 Linux/riscv64 约定：`a7=number`、`a0..a5=args`、`a0=result`、kernel error 为负 errno。共享 crate 只有 70 个已实现的 Linux number，未识别 number 一律 `-ENOSYS`。
...
1. **权限与身份竖切**：同时引入 Process credentials、VFS traversal/inode mode、exec、link 与 signal permission，删除固定 root access policy。
2. **共享映射竖切**：补齐 MAP_SHARED、page-cache coherence、共享 dirty/writeback 与 fault lifecycle。
3. **事件与网络 I/O**：在统一 OFD/poll wait seam 上实现 epoll 与 socket，而不是新增旁路 descriptor 模型。
