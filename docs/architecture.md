...
- rootfs 只有一个 BusyBox ELF inode；`/bin/init`、`/bin/sh` 与选定基础 applet 全部是指向它的 hardlink。`/etc/inittab` 只监督 `/bin/liteui-session`、network service 与 UART recovery ash；session 再统一启动 display broker、唯一 compositor、System Shell host、terminal service 与 Calculator host。完整 userspace 由本地 RSA256 签名的 `liteos-base` APK 唯一拥有，最终 `/lib/apk/db/installed` 只能由 guest 内真实 `apk.static --initdb add` 生成；host builder 不伪造 package database，也不保留未登记的第二套 runtime。
...
- `/etc/inittab` 不再分别 respawn display/terminal/application。`liteui-session` 在首个 child publication 前取得 supervision epoch，按顺序创建 activated seat/compositor socket并启动 generation；任一中途失败逆序回收，broker/compositor 退出则终止其余成员后整体重建，Shell/terminal/Calculator 退出只局部重启。`make run-gui` 默认显示 3008×1692 guest mode 的窗口化 Cocoa Display（`zoom-to-fit=off`），关闭 QEMU monitor，并把 UART 保留到 `target/run-gui-serial.log`；`make run` 仍是显式 UART 开发入口。`run/run-gui/run-gdb` 共用显式 512 MiB 客体内存基线，kernel 仍只消费 DTB 报告的动态物理内存范围。network service 的 stdout/stderr 截断写入 `/run/network-service.log`，后台 lease/renew 日志不与交互命令行竞争 UART。
...
4. **桌面竖切**：Phase 60 已完成 modern VirtIO-GPU/evdev/DRM capability seam；[Phase 61 desktop](phase-61-desktop.md) 已把 direct-DRM terminal/reference client 收敛为 session-owned 唯一 compositor，并接入 QuickJS/Solid System Shell、APK Calculator、terminal-service/TextGrid、窗口/input event 与 failure-atomic resize。真实 QEMU runtime gate 已闭合 pointer latency、damage、resize 连续性、idle CPU、RSS 与 generation recovery。下一条竖切应优先扩展有明确应用需求的公共 UI/API；只有测量证明 socket copy 或 CPU raster 已成为瓶颈时，才引入共享 buffer 或 GPU backend，禁止恢复 direct DRM client、CPU 全帧 memcpy、永久 fallback 或轮询。
...
- `make verify-runtime-desktop` 先按 kernel/bootloader/rootfs 依赖构建，再以私有 rootfs 副本启动真实
  QEMU `virt -smp 8`；host RFB client 只注入 pointer/resize，guest inspector 依据 compositor 固定诊断
  snapshot 与 procfs 判定 pointer latency/damage、resize commit/PID 连续性、RSS、idle ticks 和
  Shell/compositor generation recovery。完整执行输入与 QEMU identity 命中原子缓存时不重复运行。
