...
- `kernel/`：S-mode kernel；trap/SMP、Sv39、task/scheduler、同步读写 ext2、fd/OFD、PLIC/VirtIO block/RTC、syscall。
- `user/`：静态 `_start`、panic/output、最小 wrapper 和唯一 `/bin/init`。

当前没有 fork/clone/wait、signal、futex、IPC、`mmap`、PIE/dynamic ELF、TLS、shell、GUI 或 network userspace。不得为旧功能恢复私有 syscall 或同名近似实现；未完成标准语义时保持 `-ENOSYS`。
