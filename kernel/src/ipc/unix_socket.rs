use alloc::{collections::{BTreeMap, VecDeque}, string::{String, ToString}, sync::{Arc, Weak}, vec::Vec};
use spin::Mutex;

use crate::fs::{FileSystemError, inode::{Inode, InodeType}};
use crate::task::{current_task, block_current_and_run_next, TaskControlBlock};
...
impl UnixStreamSocket {
    pub fn new() -> Arc<Self> {
        Arc::new(Self { inner: Mutex::new(UnixStreamInner { recv: VecDeque::with_capacity(4096), peer: None, closed: false, poll_waiters: BTreeMap::new() }) })
    }
    ...
    fn wake_pollers(&self, mask: u32) {
        let mut to_wakeup: Vec<Weak<TaskControlBlock>> = Vec::new();
        {
            ...
            for (pid, (w, interests)) in inner.poll_waiters.iter() {
                if (mask & *interests) != 0 { to_wakeup.push(w.clone()); }
...
impl Inode for UnixStreamSocket {
    ...
    fn read_at(&self, _offset: u64, buf: &mut [u8]) -> Result<usize, FileSystemError> {
        ...
        loop {
            let mut got = 0usize;
            {
                let mut inner = self.inner.lock();
                while got < buf.len() {
                    if let Some(b) = inner.recv.pop_front() { buf[got] = b; got += 1; } else { break; }
                }
                if got > 0 {
                    // 可写
                    drop(inner);
                ...
                if inner.closed { return Ok(0); }
                // 无数据，阻塞
                if let Some(task) = current_task() {
                    inner.poll_waiters.insert(task.pid(), (Arc::downgrade(&task), POLLIN));
                } else {
    ...
    fn poll_mask(&self) -> u32 {
        let inner = self.inner.lock();
        let mut m = 0u32;
        if !inner.recv.is_empty() { m |= POLLIN; }
        if !inner.closed { m |= POLLOUT; }
        if inner.closed { m |= POLLHUP; }
        m
    }
...
impl UnixListener {
    pub fn new(path: &str, backlog: usize) -> Arc<Self> {
        Arc::new(Self { path: path.to_string(), backlog, queue: Mutex::new(Vec::new()), poll_waiters: Mutex::new(BTreeMap::new()) })
    }
    fn push_conn(&self, s: Arc<UnixStreamSocket>) {
        let mut q = self.queue.lock();
        if q.len() < self.backlog.max(1) { q.push(s); }
        drop(q);
        let waiters = self.poll_waiters.lock().clone();
        for (_, (w, interests)) in waiters {
            if (interests & POLLIN) != 0 {
                if let Some(t) = w.upgrade() { t.wakeup(); }
            }
        }
    }
...
impl Inode for UnixListenerInode {
    fn inode_type(&self) -> InodeType { InodeType::Device }
    fn size(&self) -> u64 { 0 }
    fn read_at(&self, _offset: u64, _buf: &mut [u8]) -> Result<usize, FileSystemError> { Err(FileSystemError::InvalidOperation) }
    fn write_at(&self, _offset: u64, _buf: &[u8]) -> Result<usize, FileSystemError> { Err(FileSystemError::InvalidOperation) }
    fn list_dir(&self) -> Result<Vec<String>, FileSystemError> { Err(FileSystemError::NotDirectory) }
    fn find_child(&self, _name: &str) -> Result<Arc<dyn Inode>, FileSystemError> { Err(FileSystemError::NotDirectory) }
    fn create_file(&self, _name: &str) -> Result<Arc<dyn Inode>, FileSystemError> { Err(FileSystemError::NotDirectory) }
    fn create_directory(&self, _name: &str) -> Result<Arc<dyn Inode>, FileSystemError> { Err(FileSystemError::NotDirectory) }
    ...
    fn register_poll_waiter(&self, interests: u32, task: Arc<TaskControlBlock>) {
        self.listener.poll_waiters.lock().insert(task.pid(), (Arc::downgrade(&task), interests));
    }
    fn clear_poll_waiter(&self, task_pid: usize) {
...
pub fn uds_listen(path: &str, backlog: usize) -> Result<Arc<UnixListenerInode>, FileSystemError> {
    ...
    if reg.contains_key(path) { return Err(FileSystemError::AlreadyExists); }
...
pub fn uds_accept(path: &str) -> Result<Arc<UnixStreamSocket>, FileSystemError> {
    ...
    drop(reg);
    loop {
        if let Some(conn) = l.pop_conn() { return Ok(conn); }
        if let Some(task) = current_task() {
            l.poll_waiters.lock().insert(task.pid(), (Arc::downgrade(&task), POLLIN));
