...
TaskManager 的唯一 indexed wait registry 拥有 wait registration，并可同时将它索引到 `(TGID,uaddr)` 和 absolute deadline。队列锁覆盖 futex 用户值比较与 `WaitMembership` 发布；wake 和 timeout 都原子删除同一 registration 及全部索引，因此不存在 compare/enqueue lost wakeup 或超时/显式唤醒双重消费。当前支持 WAIT/WAKE、PRIVATE flag 和 WAIT relative monotonic timeout，不支持 PI、requeue、bitset 或 signal interruption。
...
- signal altstack/queue/restart、futex PI/requeue/bitset/signal interruption 与完整 pthread primitive。
...
1. **阻塞 syscall 中断竖切**：在已通过的 pthread mutex/condition/timedwait 上引入 signal interruption，统一 wait cancellation 与 restart/EINTR 决策。
