...
impl VirtIODevice {
    ...
    pub fn get_status(&self) -> Result<u32, BusError> {
        self.bus.read_u32(STATUS)
    }

    pub fn set_guest_page_size(&self, size: u32) -> Result<(), BusError> {
        self.bus.write_u32(GUEST_PAGE_SIZE, size)
    }

    pub fn select_queue(&self, queue: u32) -> Result<(), BusError> {
        self.bus.write_u32(QUEUE_SEL, queue)
    }

    pub fn queue_max_size(&self) -> Result<u32, BusError> {
        self.bus.read_u32(QUEUE_NUM_MAX)
    }

    pub fn set_queue_size(&self, size: u32) -> Result<(), BusError> {
        self.bus.write_u32(QUEUE_NUM, size)
    }

    pub fn set_queue_align(&self, align: u32) -> Result<(), BusError> {
        self.bus.write_u32(QUEUE_ALIGN, align)
    }

    pub fn set_queue_pfn(&self, pfn: u32) -> Result<(), BusError> {
        self.bus.write_u32(QUEUE_PFN, pfn)
    }

    pub fn set_queue_ready(&self, ready: u32) -> Result<(), BusError> {
        self.bus.write_u32(QUEUE_READY, ready)
    }

    pub fn notify_queue(&self, queue: u32) -> Result<(), BusError> {
        self.bus.write_u32(QUEUE_NOTIFY, queue)
    }

    pub fn interrupt_status(&self) -> Result<u32, BusError> {
        self.bus.read_u32(INTERRUPT_STATUS)
    }

    pub fn interrupt_ack(&self, interrupt: u32) -> Result<(), BusError> {
        self.bus.write_u32(INTERRUPT_ACK, interrupt)
    }

    pub fn read_config_u64(&self, offset: usize) -> Result<u64, BusError> {
