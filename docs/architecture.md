...
- 每个 live Thread 都拥有固定 kernel stack，因此 topology 用物理页容量/kernel-stack 页数证明 runnable membership 上界，并在启动时为每 hart local heap 与 inbound mailbox 一次预留；local enqueue 只在 capacity pressure 时全量清理 stale generation、平时只清连续 stale root，remote delivery 只在 mailbox 满时清理，mailbox drain 每轮持锁对 snapshot 做一次全量 validation，并仅在 surviving batch 越过 local backing 时 compact heap；所有路径保留 backing capacity，wake/IRQ 路径不分配。
...
epoll evaluation 对顶层及 nested epoll 都先捕获 ctl/close notification read generation，再把 change source 与全部 OFD source 直接追加到一个 transient key builder；builder 只转移一份 key Vec 与一份 generation-guard Vec，不为每个 interest 建临时 key collection。poll、epoll 与 direct blocking 在 keys/guards 完成后共用唯一递归 adapter-preparation seam，并在进入 registry lock 前传播 nested snapshot OOM；console adapter 错误 policy 由显式 enum 保留既有 poll/direct-ignore 与 epoll-`EIO` 差异。wait-registry lock 内只排空 change token、比较 generation 并扫描 level readiness，不复制 snapshot、Arc 或 keys。change source 使用不分组的普通 registration，一次 ctl/close 先摘除全部已发布 stale waiter membership；普通 data source 仍按顶层 epoll instance 分组做 wake-one。持久 generation mismatch 继续拒绝尚未发布的并发旧 snapshot，即使 token 已被另一 waiter 消费、新 interest 尚未 ready 也强制重建 source-key set，因此 ADD/MOD/DEL/final-close 不会让 waiter 睡在旧 registration 上，OOM 也不会被折成 not-ready。LT/ET/ONESHOT、revision 与 delivery cursor 仍由 `fs::Epoll` 唯一拥有。
...
- VirtIO-GPU 2D adapter 查询第一个 enabled scanout，以连续 XRGB8888 fallback backing 同步 bootstrap。运行期使用两个 resource ID 交替执行 CREATE→ATTACH→TRANSFER→SET_SCANOUT→FLUSH→UNREF；新 backing 从提交起由 transition Arc 保活并捕获本次 mode，旧 ID/backing 只在 UNREF completion 后归还。
- 启动期在 scheduler/IRQ publication 前只允许固定命令链同步 bootstrap；运行期禁止复用自旋路径。config interrupt 与 vring interrupt 只发布 display softirq；deferred context 异步 GET_DISPLAY_INFO，并把变化作为候选 mode 交给 DRM。DRM 先分配同尺寸零填充 fallback，再确认 adapter/connector mode 的两阶段提交；OOM 精确保留旧代际。通用 display seam 不泄漏 VirtIO command/resource/descriptor。
- DRM owner 在 composition root 接收 display seam 与统一 Pipe notification。completion lock 与 adapter controlq 固定同一锁序；hardirq 只确认并发布 softirq，每个 softirq 有界推进一个 stage，最终 fence 才发布 active framebuffer、page-flip event 与 Pipe edge。同步 SETCRTC/RMFB 在 syscall/task wait seam 阻塞，DRM 只提供防 lost-wakeup 的 wait token/event source，因此没有 `drm ↔ task` 循环依赖。
- devfs 发布标准 Linux primary node `/dev/dri/card0`（226:0）。query 支持 VERSION、GET_CAP、GETRESOURCES、GETCRTC、GETENCODER 与 GETCONNECTOR；CRTC/encoder/virtual-connector object identity 固定，connector preferred mode 动态投影已提交的 adapter display-info，active CRTC 继续携带完成时捕获的 mode。
...
- device-wide framebuffer map 支持 legacy ADDFB/GETFB/RMFB 与 XRGB8888 single-plane ADDFB2；framebuffer object 独立保活 GEM backing。primary node 第一个 open 自动成为 master，SET/DROP_MASTER 遵循 current/was-master 与 effective-root 权限边界；SETCRTC/PAGE_FLIP 只允许 current master。active RMFB 与 OFD close 都恢复 fallback，close cleanup 不分配。
- PAGE_FLIP 支持 flags=0 与 `DRM_MODE_PAGE_FLIP_EVENT`；completion 在每-OFD 固定 4 KiB event space 中无分配发布完整 `drm_event_vblank`，read/readv、pselect/ppoll/epoll 共用 Pipe notification、monotonic timestamp 与 level recheck。target/async flags、auth/lease、vblank wait 与 atomic KMS 尚未发布。
...
- rootfs 固定的 `/bin/liteos-terminal` 是唯一显示 terminal consumer：查询标准 DRM/KMS topology，建立一个 XRGB8888 dumb framebuffer，以 page-flip completion 驱动刷新；扫描 keyboard evdev，使用 PTY 启动新的 login-style ash。终端校验并 mmap 固定 Spleen 2.2.0 PSF2，以原生 16×32 位图逐像素渲染，默认 1280×800 mode 精确得到 80×25；字体及许可证由 rootfs 固定资产拥有，内核/DRM 不感知字形。输出唤醒后以 nonblocking read 合并当前 PTY burst，每批只渲染/flip 一次；为弥补当前尚无 kobject uevent/netlink hotplug consumer，空闲 poll 最长一秒后只重查一次标准 GETCONNECTOR，mode 变化时先在同一 DRM fd 建立并 SETCRTC 新 dumb framebuffer，成功后回收旧 framebuffer，再更新 PTY winsize；`TIOCSWINSZ` 仅在值变化时向 foreground group 发送 `SIGWINCH`。当前是紧凑 ANSI/ASCII console，不是 compositor；图形程序直接接管 DRM master/会话的协议仍属于下一阶段。
...
- rootfs 只有一个 BusyBox ELF inode；`/bin/init`、`/bin/sh` 与选定基础 applet 全部是指向它的 hardlink，`/etc/inittab` 同时监督 UART ash 与固定 musl PIE `/bin/liteos-terminal`。完整 userspace 由本地 RSA256 签名的 `liteos-base` APK 唯一拥有，最终 `/lib/apk/db/installed` 只能由 guest 内真实 `apk.static --initdb add` 生成；host builder 不伪造 package database，也不保留未登记的第二套 runtime。
- `/etc/inittab` 以独立 `respawn` action 立即启动 UART login-style root ash、DRM/evdev/PTY 显示 ash 与前台 `udhcpc`，不保留 `askfirst`/Enter 激活入口；任一 shell exit 或 session teardown 后由 init 恢复对应 console。network service 的 stdout/stderr 截断写入 `/run/network-service.log`，后台 lease/renew 日志不与交互命令行竞争 UART。
...
4. **显示与输入 I/O**：modern VirtIO-GPU 2D、single-scanout fallback、六阶段 IRQ-driven resource 切换、标准 `/dev/dri/card0` query、per-file GEM/dumb mmap、framebuffer lifecycle、master ownership 与 page-flip event/readiness 已闭环；VirtIO keyboard→evdev→Unix98 PTY/devpts→ash→固定用户态 terminal→DRM 的竖切也已闭环。下一步建立 display-session/DRM-master handoff 与可独立退出的 graphics client 生命周期，再由第一个真实 2D/3D consumer 决定 atomic KMS、render-node 或 input-seat 的最小扩展；禁止暴露 VirtIO 私有 ioctl、CPU 全帧 memcpy 或恢复运行期自旋。
