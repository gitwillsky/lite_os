...
- 42 个 Linux/riscv64 syscall number；UART RX IRQ 通过统一 console OFD/wait registry 支撑真实输入，文件入口统一经过 fd/OFD 与 VFS，未实现编号返回 `-ENOSYS`。
...
该目标先完成 musl gate，再校验 BusyBox 官方 tarball SHA-256，应用唯一 `user/busybox.config` 与 `user/inittab`，构建 RISC-V 静态 `ET_EXEC` 和单 inode hardlink applet rootfs。gate 冷启动 BusyBox init，等待 UART `askfirst`，注入命令并要求 ash 计算/输出 `LITEOS_BUSYBOX_SHELL_42`；同时检查无 dynamic interpreter、无 W+X LOAD、用户栈不可执行。BusyBox 源码和二进制不进入仓库。该路径证明当前交互 tracer bullet，不表示 pipe、job control、完整 TTY 或全部 applet 已成立。
