...
U-mode 使用 Linux/riscv64 约定：`a7=number`、`a0..a5=args`、`a0=result`、kernel error 为负 errno。共享 crate 只有 30 个已实现的 Linux number，未识别 number 一律 `-ENOSYS`。
...
1. **process 竖切**：`clone/fork + wait4 + exit_group`，同时加 parent/child owner 和最小 exit record，不复活完整 zombie TCB。
2. **thread/futex 竖切**：`clone` thread flags、TLS、`set_tid_address`、futex wait/wake、robust cleanup 共用唯一 lost-wakeup 协议。
3. **signal 竖切**：`rt_sigaction/rt_sigprocmask/rt_sigreturn/tgkill`、riscv64 rt frame、pending selection 和 syscall interruption 一次成立。
4. **musl 验收竖切**：在 fd、VMA、TLS、futex、signal 成立后固定一个 musl commit，以静态非线程程序开始，再扩展 pthread/dynamic linker。
5. **设备扩展**：只在标准 fd/ioctl/mmap/poll UAPI 成立后加入 console/network/GPU/input；真实硬件另行证明 PLIC topology 和 DMA coherence。
