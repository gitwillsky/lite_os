...
BusyBox 官方 release 1.37.0 已固定 tarball SHA-256、唯一 `init + ash`/基础 applet config 与 inittab。默认 `make build` 构造并静态检查单 BusyBox inode/hardlink rootfs，`make run` 直接启动该镜像；仓库不再存在 Rust init/runtime 或另一条 rootfs 路径。选中 applet 还包括 `ifconfig/route/nc/netstat/udhcpc/wget`：QEMU user-net 下 `udhcpc` 经标准 AF_PACKET 与 socket option 获取 lease，唯一 lease script 通过 ioctl/route 配置 `eth0` 并生成 musl 消费的 `/etc/resolv.conf`；固定动态 musl probe 与 `wget` 分别验证 DNS resolver 和 TCP/HTTP client。`nc -u` 通过 `recvmsg/MSG_PEEK/IP_PKTINFO` 完成双向 UDP，`nc` 的 active connect 与 passive listen/accept 完成双向 TCP stream。现有 gate 还验证 ash 算术、pipeline、重定向、后台 wait、可执行 script、process-group kill、`ps/free/uptime/top/df/uname/arch/date` 系统可观测性、64 次动态 exec/exit 后的内存回收、跨冷启动持久化、foreground Ctrl-Z/jobs/bg/fg/Ctrl-C，以及 `stty` 驱动的后台 read/SIGTTIN 与 `TOSTOP` write/SIGTTOU 停止-恢复链。当前不证明未选中 applet。
...
- vDSO 与 `AT_HWCAP`。
- IPv6、raw IP/ICMP userspace socket、Ethernet ARP packet protocol、多 NIC/network namespace、TCP keepalive/linger policy、GPU/input user device ABI，以及完整 VMIN/VTIME。
