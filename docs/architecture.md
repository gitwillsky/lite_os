...
- `Process`：Arc-owned TGID、AddressSpace、cwd opened entry、FileDescriptorTable，由 thread group 共享。相对 pathname 直接从 cwd opened entry 解析；`getcwd` 从 VFS-owned parent/name/deleted 链生成 raw absolute path，不保留可能随 rename 漂移的 path cache。
