...
- 首个 graphics consumer 是紧凑常驻 Rust 2D reference client：libseat+libdrm、两个 dumb buffer、geometry page-flip event、固定容量 DIRTYFB pointer damage、software geometry/pointer、keyboard/mouse 与 resize/hotplug。pointer report 只传输旧、新光标矩形；geometry flip 先 DIRTYFB 预同步 inactive buffer，再复用两槽 resident GPU resource 做 SET_SCANOUT→FLUSH，禁止触发逐帧 CREATE/ATTACH/UNREF；idle 必须阻塞，render 上限 60 fps；不引入 toolkit、compositor、Mesa/GBM/EGL、字体、图片或 3D。
