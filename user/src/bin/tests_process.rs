...
use user_lib::{fork, execve, wait_pid, wait_pid_nb, getpid, gettid, get_args, 
...
fn test_process_creation(stats: &mut TestStats) {
    ...
    let euid = geteuid();
    let egid = getegid();
    test_info!("权限信息 - UID: {}, GID: {}, EUID: {}, EGID: {}", uid, gid, euid, egid);
...
fn test_fork_basic(stats: &mut TestStats) {
    ...
    if fork_pid == 0 {
        ...
        test_assert!(waited_pid == fork_pid, "wait_pid 返回错误: {} != {}", waited_pid, fork_pid);
...
fn test_multiple_forks(stats: &mut TestStats) {
    ...
    for i in 0..num_children {
        ...
        test_assert!(status == expected_status as i32, "子进程 {} 退出码错误: {} != {}", 
...
fn test_wait_variants(stats: &mut TestStats) {
    ...
    if fork_pid == 0 {
        // 子进程先睡眠一小段时间
        let sleep_time = user_lib::TimeSpec { tv_sec: 0, tv_nsec: 100_000_000 }; // 100ms
...
fn test_permission_syscalls(stats: &mut TestStats) {
    ...
    test_info!("原始权限 - UID:{} GID:{} EUID:{} EGID:{}", orig_uid, orig_gid, orig_euid, orig_egid);
    ...
    test_info!("权限设置结果 - setuid:{} setgid:{} seteuid:{} setegid:{}", ret1, ret2, ret3, ret4);
