...
pub fn sys_open(path: *const u8, flags: u32) -> isize {
    ...
    if let Some(task) = current_task() {
        ...
        let file_desc = Arc::new(FileDescriptor::with_path(inode, flags, path_str));
...
/// mkdirat - 在指定目录下创建目录
pub fn sys_mkdirat(dirfd: i32, path: *const u8, mode: u32) -> isize {
    let token = current_user_token();
    let path_str = match translated_c_string(token, path) {
        Ok(path) => path,
...
/// ppoll - 带信号掩码的poll
pub fn sys_ppoll(fds_ptr: *mut u8, nfds: usize, timeout: *const u8, sigmask: *const u64) -> isize {
    // 如果有信号掩码，暂时保存当前的并设置新的
    let old_mask = if !sigmask.is_null() && !timeout.is_null() {
        if let Some(task) = current_task() {
            let token = task.mm.user_token();
            let new_mask_val = unsafe { *translated_ref_mut(token, sigmask as *mut u64) };
            let mut signal_state = task.signal_state.lock();
            let old = signal_state.get_blocked();
...
/// fchownat - 修改指定目录下文件的所有者
pub fn sys_fchownat(dirfd: i32, path: *const u8, uid: u32, gid: u32, flags: i32) -> isize {
    let token = current_user_token();
    let path_str = match translated_c_string(token, path) {
        Ok(path) => path,
        Err(_) => return -36, // ENAMETOOLONG
...
/// faccessat - 检查指定目录下文件的访问权限
pub fn sys_faccessat(dirfd: i32, path: *const u8, mode: i32, flags: i32) -> isize {
    let token = current_user_token();
    let path_str = match translated_c_string(token, path) {
        Ok(path) => path,
        Err(_) => return -36, // ENAMETOOLONG
