...
- `Process`：TGID、AddressSpace、cwd、FileDescriptorTable。
...
TaskManager 的单一 process graph 拥有单调 PID 分配、parent edge，以及每个 PID 的 live task 或最小 exit record。fork-shaped `clone` eager 深拷贝 VMA/frame/cwd/fd table entry，fd entry 继续共享 OFD；child 从相同 PC 返回零。exit 释放完整 TCB，只在 graph 留 `pid,parent,status`，阻塞 `wait4` copyout 成功后消费该 record。orphan 只改写 graph 中的 parent edge到 PID 1，不保留第二套 child collection。
...
U-mode 使用 Linux/riscv64 约定：`a7=number`、`a0..a5=args`、`a0=result`、kernel error 为负 errno。共享 crate 只有 32 个已实现的 Linux number，未识别 number 一律 `-ENOSYS`。
...
- multi-thread、TLS、clone thread/tid-pointer/vfork 语义。
...
1. **thread/futex 竖切**：`clone` thread flags、TLS、`set_tid_address`、futex wait/wake、robust cleanup 共用唯一 lost-wakeup 协议。
2. **signal 竖切**：`rt_sigaction/rt_sigprocmask/rt_sigreturn/tgkill`、riscv64 rt frame、pending selection 和 syscall interruption 一次成立。
3. **musl 验收竖切**：在 fd、VMA、TLS、futex、signal 成立后固定一个 musl commit，以静态非线程程序开始，再扩展 pthread/dynamic linker。
4. **设备扩展**：只在标准 fd/ioctl/mmap/poll UAPI 成立后加入 console/network/GPU/input；真实硬件另行证明 PLIC topology 和 DMA coherence。
