...
- 40 个 Linux/riscv64 syscall number；文件入口统一经过 fd/OFD 与 VFS，未实现编号统一返回 `-ENOSYS`。
...
- BusyBox：官方 release `1.37.0`，作为最终唯一 `init + ash` userspace consumer；当前只纳入静态构建/ELF gate，尚未进入启动镜像。
...
固定 BusyBox 1.37.0 静态构建可单独执行：

```bash
make verify-busybox
```

该目标先完成 musl gate，再校验 BusyBox 官方 tarball SHA-256，应用仓库内唯一 `user/busybox.config`，构建 RISC-V 静态 `ET_EXEC` 并检查无 dynamic interpreter、无 W+X LOAD、用户栈不可执行。BusyBox 源码和二进制不进入仓库；当前 gate 不表示 BusyBox 已能由 LiteOS 启动。
...
本仓库不维护、不修正、不执行测试用例。统一运行 `make verify`；它执行 AST 架构围栏、workspace/组件构建、Clippy、ELF 静态检查、`-smp 1/3/8` Rust init 冷启动、固定 musl pthread consumer 冷启动，以及固定 BusyBox 静态构建/ELF gate。各阶段的证据保存在 `docs/phase-*.md`。
