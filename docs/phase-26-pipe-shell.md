...
新增 `pipe2(59)`、`readv(65)`、`rt_sigsuspend(133)` 和固定 root identity getter 174-177。`readv/writev` 共享 OFD 后端，Pipe readv 一次读取后 scatter，已有进度不会为后续 iovec 再阻塞。`rt_sigsuspend` 与 `rt_sigtimedwait` 共用 Signal membership，但不消费 pending bit；signal frame 恢复 suspend 前 mask。
