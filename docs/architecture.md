...
固定 musl v1.2.6 的无 `PT_TLS` 静态 pthread smoke 已作为真实 ABI consumer 启动：musl 从 initial auxv 初始化内建主线程区，使用 `PROT_NONE -> mprotect(RW)` 建立带 guard 的 child stack，以 Linux pthread clone flags 发布 TLS/parent-TID/clear-child-tid，并经 private futex 与 thread exit 完成 `pthread_join`。这只证明单个 `pthread_create/join` 路径，不改变上述通用边界。
...
1. **pthread 同步竖切**：从已通过的单个 create/join 扩展到 mutex/cond 与 timeout，由真实 musl 路径驱动 futex timeout 和 signal interruption。
