...
Signal disposition 属于共享 Process；mask、standard-signal pending bitset 与至多一条 syscall replay record 属于 Thread。`tgkill` 先合并 pending bit；未屏蔽、非忽略 signal 会取消 deadline/futex/child wait 并以 `Interrupted` 结果恢复 task。trap/syscall 边界把可重启结果暂存为 userspace `EINTR`，同时保存原 `a0..a5/a7/ecall PC`；实际交付 handler 含 `SA_RESTART` 时，signal frame 才保存 replay context，否则保存 `EINTR` context。trap return 选择最低未屏蔽 signal，在普通用户栈构造 Linux RV64 `siginfo + ucontext + sigcontext`，保存完整 GPR/FP state，并把 RA 指向独立 U|RX signal-return trampoline。`rt_sigreturn` 恢复原 mask、寄存器、FP 与 PC。当前 blocking `wait4` 和无 timeout 的 futex WAIT 可重启；带 relative timeout 的 futex WAIT 与 `nanosleep` 保持 `EINTR`，避免错误延长 timeout。无 altstack、queued realtime value、SIGCHLD 生成和 process-directed kill。
...
每个 Process 拥有唯一 `FileDescriptorTable`；fd entry 保存 `FD_CLOEXEC`，dup 后共享同一 OFD 的 offset/status flags。PID 1 的 `0/1/2` 在建进程时安装为 console OFD，`read/write/writev/close/dup/fcntl` 不存在 console syscall 旁路。console read 从 UART IRQ 固定容量 ring 取数据；空 ring 通过统一 indexed wait registry 阻塞，并由 deferred softirq wake，不轮询 task table。
...
DTB 同时提供 16550 MMIO range 与 PLIC IRQ。kernel identity-map 该 range，UART driver 唯一拥有固定 1024-byte RX ring；hardirq 只 volatile drain RBR、在 ring 满时继续 ack 并丢弃超额输入，然后发布 per-hart console softirq。task layer 在 deferred context 消费统一 Console wait membership。当前只是 raw byte stream；尚无 termios、canonical mode、echo、foreground process group 或 `/dev/console` inode。
...
BusyBox 官方 release 1.37.0 已固定 tarball SHA-256、唯一 `init + ash`/基础 applet config 与 inittab。独立 gate 构造单 BusyBox inode/hardlink rootfs，冷启动 init，等待 UART `askfirst`，注入命令并由 ash 输出 `LITEOS_BUSYBOX_SHELL_42`。默认 `make build` 尚未切换，且该 tracer bullet 不证明 pipe、job control、完整 TTY 或全部 applet。

## 13. Syscall ABI

U-mode 使用 Linux/riscv64 约定：`a7=number`、`a0..a5=args`、`a0=result`、kernel error 为负 errno。共享 crate 只有 41 个已实现的 Linux number，未识别 number 一律 `-ENOSYS`。
...
- network/GPU/input user device ABI，以及 console termios/device-node/foreground-group 语义。
