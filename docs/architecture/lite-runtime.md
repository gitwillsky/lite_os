# 图形会话与 LiteUI 运行时（lite-runtime）当前架构
...
- `lite-runtime`（crate `lite-runtime`，lib 名 `lite_runtime`）是通用 GUI/JS 运行时**库**——像
  ...
  `GrowingFile` 流式解码）。它自身不产出可执行文件。
- 每个窗体程序是链接该库的**独立二进制** `/bin/<id>`：`desktop`（`--desktop` 角色）、
  `file-manager`、`my-computer`、`terminal`、`music-player`。每次启动建立一个进程、一个 QuickJS VM、
