...
fn production_music_player_uses_standard_range_and_readable_media_controls() {
    ...
    // The Search view exposes no range inputs; seek and volume ranges live in
    // the Now-Playing view (verified via the source bundle's RangeInput usage
    // and the runtime range labels asserted above).
    assert_eq!(
        count_range_inputs(&scene),
        0,
        "search surface renders no range inputs before a track is playing"
    );
}
