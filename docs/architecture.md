...
- 2 MiB bootstrap buddy arena 只打破启动期循环依赖；frame allocator 发布后，kernel heap 按需转移连续或 recycled 物理页扩容，不设编译期容量上限。recycler 使用页首 next PPN 组成严格递增 intrusive list：连续 allocation 单次线性查找，连续 `FrameTracker` Drop 在一次 IRQ-safe lock 内 batch 回收。
...
U-mode 使用 Linux/riscv64 约定：`a7=number`、`a0..a5=args`、`a0=result`、kernel error 为负 errno。共享 crate 只有 136 个已接入的 Linux number，未识别 number 一律 `-ENOSYS`。
