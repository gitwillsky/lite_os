...
4. **显示与输入 I/O**：modern VirtIO-GPU 2D、精确 SG backing、IRQ-driven scanout/damage/disable、标准 `/dev/dri/card0`、DIRTYFB、kobject hotplug、GEM/dumb mmap、master 与 page-flip event 已闭环；VirtIO keyboard→evdev→Unix98 PTY/devpts→ash→Rust terminal→DRM 的无轮询竖切也已闭环。下一步建立 display-session/DRM-master handoff 与可独立退出的 graphics client 生命周期，再由首个真实 2D/3D consumer决定 atomic KMS、render-node、buffer sharing 或 input-seat 的最小扩展；禁止暴露 VirtIO 私有 ioctl、CPU 全帧 memcpy、永久 fallback 或恢复运行期自旋。
