...
1. **TTY/job-control 生命周期加固**：实现 background TTY read/write 的 SIGTTIN/SIGTTOU action，并补齐 PID 1 与 credential signal policy。
