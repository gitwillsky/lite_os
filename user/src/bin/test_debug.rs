...
use user_lib::webcore::css::{StandardCSSParser, CSSParser, ComputedStyle, StyleComputer, ComputationContext};
...
fn test_full_render() {
    ...
    for (i, cmd) in result.commands.iter().enumerate() {
        match cmd {
            user_lib::webcore::paint::DrawCommand::DrawText { x, y, text, color, size } => {
                println!("  Command {}: DrawText at ({}, {}) '{}' size={} color={:#x}", 
                    i, x, y, text, size, color);
            }
            user_lib::webcore::paint::DrawCommand::FillRect { x, y, width, height, color } => {
                println!("  Command {}: FillRect at ({}, {}) {}x{} color={:#x}", 
...
fn print_dom_structure(node: &user_lib::webcore::html::DomNode, depth: usize) {
    ...
    if node.tag.is_empty() {
        ...
        println!("{}[{}]{} children={}", indent, node.tag, attrs, node.children.len());
...
fn print_style_tree(styled: &user_lib::webcore::style::StyledNode, depth: usize) {
    ...
    println!("{}[{}] color=({},{},{}) bg_color=({},{},{}) font_size={}", 
        indent, tag,
...
fn print_layout_structure(layout: &user_lib::webcore::layout::LayoutBox, depth: usize) {
    ...
    println!("{}Layout: x={} y={} w={} h={} color={:#x} bg_color={:#x}", 
        indent, layout.rect.x, layout.rect.y, layout.rect.w, layout.rect.h,
        color.to_u32(), bg_color.to_u32());
