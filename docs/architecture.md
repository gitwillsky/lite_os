...
- `Process`：Arc-owned TGID、AddressSpace、cwd、FileDescriptorTable，由 thread group 共享。
...
TaskManager 的唯一 indexed wait registry 拥有 wait registration，并可同时将它索引到 `(TGID,uaddr)` 和 absolute deadline。队列锁覆盖 futex 用户值比较与 `WaitMembership` 发布；wake、timeout 和 signal interruption 都原子删除同一 registration 及全部索引。blocking path 在 owner lock 内复查 deliverable signal，因此不存在 compare/enqueue、signal/enqueue lost wakeup 或双重消费。当前支持 WAIT/WAKE、PRIVATE flag、WAIT relative monotonic timeout 和 thread-directed signal interruption，不支持 PI、requeue 或 bitset。

Signal disposition 属于共享 Process；mask 与 standard-signal pending bitset 属于 Thread。`tgkill` 先合并 pending bit；未屏蔽、非忽略 signal 会取消 deadline/futex/child wait 并以 `Interrupted` 结果恢复 task。trap return 选择最低未屏蔽 signal，在普通用户栈构造 Linux RV64 `siginfo + ucontext + sigcontext`，保存完整 GPR/FP state，并把 RA 指向独立 U|RX signal-return trampoline。`rt_sigreturn` 恢复原 mask、寄存器、FP 与 PC。当前支持 `tgkill`、`rt_sigaction`、`rt_sigprocmask`、`rt_sigreturn` 与阻塞 syscall `EINTR`；无 `SA_RESTART`、altstack、queued realtime value、SIGCHLD 生成和 process-directed kill。
...
每个 Process 拥有唯一 `FileDescriptorTable`；fd entry 保存 `FD_CLOEXEC`，dup 后共享同一 OFD 的 offset/status flags。PID 1 的 `0/1/2` 在建进程时安装为 console OFD，`read/write/close/dup/fcntl` 不存在 console syscall 旁路。
...
固定 musl v1.2.6 的无 `PT_TLS` 静态 pthread consumer 已作为真实 ABI consumer 启动：musl 从 initial auxv 初始化内建主线程区，使用 `PROT_NONE -> mprotect(RW)` 建立带 guard 的 child stack，以 Linux pthread clone flags 发布 TLS/parent-TID/clear-child-tid，并经 private futex 完成 `pthread_join`、mutex/condition/timedwait，再以 handler + `tgkill` 验证 signal-interrupted futex、`nanosleep` 和 `waitpid`。这只证明固定 consumer 触发的路径，不改变上述通用边界。

## 13. Syscall ABI

U-mode 使用 Linux/riscv64 约定：`a7=number`、`a0..a5=args`、`a0=result`、kernel error 为负 errno。共享 crate 只有 39 个已实现的 Linux number，未识别 number 一律 `-ENOSYS`。
...
- signal altstack/queue/`SA_RESTART`、futex PI/requeue/bitset 与完整 pthread primitive。
- pipe、socket、poll/epoll 与通用设备 fd/ioctl/mmap UAPI。
...
1. **syscall restart 竖切**：在已通过的 wait cancellation/`EINTR` 上引入内部 restart code 与 `SA_RESTART` 决策，不向 userspace 泄漏内核内部 errno。
2. **动态装载竖切**：静态线程成立后再设计 `PT_TLS/ET_DYN/PT_INTERP`、relocation 与 file mapping，不与静态路径并行维护半实现 loader。
3. **设备扩展**：只在标准 fd/ioctl/mmap/poll UAPI 成立后加入 console/network/GPU/input；真实硬件另行证明 PLIC topology 和 DMA coherence。
