...
impl Inode for DevRoot {
    ...
    fn size(&self) -> u64 { 0 }
    fn read_at(&self, _offset: u64, _buf: &mut [u8]) -> Result<usize, FileSystemError> { Err(FileSystemError::IsDirectory) }
    fn write_at(&self, _offset: u64, _buf: &[u8]) -> Result<usize, FileSystemError> { Err(FileSystemError::IsDirectory) }
    fn list_dir(&self) -> Result<Vec<String>, FileSystemError> { Ok(vec!["input".to_string()]) }
    ...
    fn create_file(&self, _name: &str) -> Result<Arc<dyn Inode>, FileSystemError> { Err(FileSystemError::PermissionDenied) }
    fn create_directory(&self, _name: &str) -> Result<Arc<dyn Inode>, FileSystemError> { Err(FileSystemError::PermissionDenied) }
    fn remove(&self, _name: &str) -> Result<(), FileSystemError> { Err(FileSystemError::PermissionDenied) }
    fn truncate(&self, _size: u64) -> Result<(), FileSystemError> { Err(FileSystemError::InvalidOperation) }
    fn sync(&self) -> Result<(), FileSystemError> { Ok(()) }
}
