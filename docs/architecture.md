...
dirty page 数只由 `CachedPage` 状态转换投影；达到物理页容量四分之一后，新的 shared writable mapping 按每 64 页一次的固定 cadence 在发布 PTE 前请求前台回收。单一跨 hart gate 合并同一高水位尝试，保证 ext2 journal staging 仍有确定余量，并避免多个 hart 重复扫描 owner 或提交 FLUSH；它不建立后台 worker，也不成为第二份 page 状态。
...
- rootfs 固定的 `/bin/liteos-terminal` 是唯一 display-session/terminal owner：查询标准 DRM/KMS topology、扫描 keyboard evdev、nonblocking 回放 `/dev/kmsg` 启动记录，再在第一次 SETCRTC 前把完整首帧绘入唯一 XRGB8888 dumb framebuffer，最后通过 PTY 启动 login-style ash；用户不会先看到空白 scanout。终端校验并 mmap 固定 Spleen 2.2.0 PSF2，以原生 16×32 位图逐像素渲染；QEMU GUI 初始 1920×1200 mode 的单用户 framebuffer 为 8.79 MiB，比原 2160×1400 降低约 24%，运行期 mode 仍以 connector 为准。cell 增加一位 owner-local dirty state，PTY burst 只重绘发生变化的 cell 和旧/新 cursor，仍每批只提交一次 page flip；不复制第二帧。空闲 poll 最长一秒后只重查一次标准 GETCONNECTOR，mode 变化时建立并预渲染同一终端状态的新 dumb framebuffer，SETCRTC 成功后才回收旧 framebuffer并更新 PTY winsize；`TIOCSWINSZ` 仅在值变化时向 foreground group 发送 `SIGWINCH`。init 是 session restart policy 的唯一 owner，进程内部不再维护第二个无限重试循环。当前仍是紧凑 ANSI/ASCII console，不是 compositor；图形程序直接接管 DRM master/会话的协议属于下一阶段。
...
- `/etc/inittab` 先以独立 `respawn` action 启动 display-session，再启动前台 `udhcpc` 与 UART login-style root ash，不保留 `askfirst`/Enter 激活入口；任一 shell exit 或 session teardown 后由 init 恢复对应 console。`make run-gui` 默认只显示全屏 Cocoa Display，关闭 QEMU monitor，并把 UART 保留到 `target/run-gui-serial.log`；`make run` 仍是显式 UART 开发入口。network service 的 stdout/stderr 截断写入 `/run/network-service.log`，后台 lease/renew 日志不与交互命令行竞争 UART。
