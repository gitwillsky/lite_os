...
impl Display {
    ...
    pub fn damage(&mut self, rectangle: Rect) {
        for buffer in self.buffers.iter_mut().flatten() {
            buffer.dirty = Some(match buffer.dirty {
                Some(dirty) => dirty.union(rectangle),
                None => rectangle,
            });
        }
    }
    ...
    pub fn has_damage(&self) -> bool {
        self.buffers
            .iter()
            .any(|buffer| buffer.as_ref().is_some_and(|buffer| buffer.dirty.is_some()))
    }
    ...
    pub fn present(&mut self, scene: &Scene) -> Result<bool, ()> {
        if self.flip_pending {
            return Ok(false);
        }
        ...
        scene.render(buffer.pixels, buffer.pitch, dirty);
        if unsafe {
            ffi::drmModePageFlip(
                self.fd,
                self.crtc_id,
