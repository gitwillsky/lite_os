...
1. **musl 验收竖切**：补齐当前明确列出的 VMA/futex/clone/signal 缺口后固定一个 musl commit，以静态非线程程序开始，再扩展 pthread/dynamic linker。
2. **设备扩展**：只在标准 fd/ioctl/mmap/poll UAPI 成立后加入 console/network/GPU/input；真实硬件另行证明 PLIC topology 和 DMA coherence。
