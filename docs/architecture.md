...
当前只实现 anonymous private eager mapping，支持地址 hint 与 `MAP_FIXED_NOREPLACE`。无 `PROT_NONE`、destructive `MAP_FIXED`、file/shared mapping、COW 或 lazy fault。
...
TaskManager 的单一 process graph 拥有单调 PID/TID 分配、parent edge，以及每个 TGID 的 live thread collection 或最小 exit record。fork-shaped clone 深拷贝 Process；thread-shaped clone 共享 Arc<Process>，使用独立 kernel stack/trap context、用户 stack 与 TLS `tp`。单线程 exit 只回收该 Thread，最后一个 Thread 才提交 process exit record。clear-child-tid 和 robust-list cleanup 在退出发布前完成。

Futex queue 以 `(TGID,uaddr)` 为 key；队列锁覆盖用户值比较和 `WaitMembership::Futex` 发布，wake 消费同一个 token，因此不存在 compare/enqueue 间的 lost wakeup。当前只支持无 timeout 的 WAIT/WAKE（含 PRIVATE flag），不支持 PI、requeue 或 signal interruption。

Signal disposition 属于共享 Process；mask 与 standard-signal pending bitset 属于 Thread。trap return 选择最低未屏蔽 signal，在普通用户栈构造 Linux RV64 `siginfo + ucontext + sigcontext`，保存完整 GPR/FP state，并把 RA 指向独立 U|RX signal-return trampoline。`rt_sigreturn` 恢复原 mask、寄存器、FP 与 PC。当前支持 `tgkill`、`rt_sigaction`、`rt_sigprocmask`、`rt_sigreturn`；无 altstack、queued realtime value、阻塞 syscall interruption/restart、SIGCHLD 生成和 process-directed kill。
...
固定 musl v1.2.6 的无 `PT_TLS` 静态 smoke 已作为真实 ABI consumer 启动：musl 从 initial auxv 初始化内建主线程区，经 `set_tid_address` 进入 `main`，并实际使用 `sysconf/getpid/malloc/free/clock_gettime/write/exit_group`。这只证明该固定 consumer 的静态路径，不改变上述通用边界。
...
- signal altstack/queue/restart、futex timeout/PI/requeue 与完整 pthread primitive。
- pipe、socket、poll/epoll 与通用设备 fd/ioctl/mmap UAPI。
- journal、突然断电后的跨块原子恢复、symlink resolution、mount namespace。
- file/shared mapping、COW、lazy paging、`PROT_NONE` 与 destructive `MAP_FIXED`。
...
1. **musl pthread 竖切**：从已通过的固定静态 consumer 扩展到单个 `pthread_create/join`，只按真实调用链补齐缺口。
