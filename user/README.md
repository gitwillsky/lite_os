...
| `console-session/` | 旧单进程 TUI 会话，已从 inittab 退役 | 等待第四期移除，不再发布接口 |
| `diagnostics/` | `cputest`, `memtest`, `cachetest` multicall executable | bounded product diagnostics |

`desktop` 是 DRM master 与输入的唯一 owner：首个 open `/dev/dri/card0` 取得 master，握手时经
SCM_RIGHTS 把同一 OFD 共享给客户端；客户端在其上 CREATE_DUMB 并把 handle 交给桌面合成，handle
的 DESTROY 只由桌面执行。`terminal` 继承原 console-session 的 ANSI parser 与 renderer，是
terminal model 的唯一 owner。UART shell 仍是 BusyBox init 拥有的独立恢复路径。
