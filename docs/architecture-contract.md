...
| per-Thread nice/vruntime、active CPU slice/dispatch weight、累计 runtime 与 legacy scheduler policy/reset-on-fork | Sched policy lock；fork/clone 只经 `Sched::forked` 派生，exec 保留 |
...
| process comm/创建时刻、mm argument range、thread runtime 与 run state | Process、MemorySet、SchedulingEntity；Process runtime 聚合保留 exited Thread，procfs 只读取 owner 快照或 mm 中的实时 argv bytes |
...
| `kernel/src/memory/mm.rs` | 1074 | `memory::MemorySet` | 核心 VMA 表示、页表提交与 kernel/system mapping 仍共享底层 PageTable/frame 不变量；mapping request、mmap、user-copy、shared/private area 与 futex key lifecycle 已下沉 | 提取不泄漏 PageTable/frame 的 kernel mapping 与 VMA mutation 深 module 后下调额度 |
...
- `memory::mm::mmap` 拥有 `PageFaultAccess/Outcome` 及其 retry postcondition；`memory::mm::user_access` 唯一拥有完整 range fault-in、权限证明与 copy。`prepare_user_read` 只向 sibling `futex_key` 开放，使 key 归一化前取得同一 `U|R` 证明；write prepare 保持 private，因为 copyout/CAS 是唯一调用者。prepare 必须以已命中的 `user_page` 或 `PageFaultOutcome::Handled` 完成逐页权限证明，禁止恢复独立 `validate_user_range` 或 fault 后重复翻译；额外 validation pass 会让所有 user-copy 对每个页重复一次 Sv39 page-table walk，却不增加失败原子性。
...
- `model::scheduling::Sched::new` 只向 Process/Thread constructor 开放；`begin_runtime` 只由 `switch_to_task` 开始 active CPU slice，`checkpoint_runtime` 只由本 hart timer deferred consumer 提交已消耗增量而不结束 slice，`finish_runtime` 只由当前 task 的 block/preempt/vfork/exit orchestration 提交剩余增量并结束 slice。checkpoint 必须同步推进 Thread/Process/hart 累计与 Q10 vruntime，并保留 dispatch nice 快照；若只刷新 per-hart counter，会复制 runtime owner并在后续 finish 重复计费。`TaskControlBlock::cpu_runtime_snapshot` 只供 `clock_gettime` 与 same-thread-group procfs 只读投影 checkpoint 后的 active delta，不提交 runtime；其他 procfs target 读取 Process 聚合 owner。nice scaling 固定使用 Linux v7.1 的 40 项 reciprocal 表并在热路径以乘法/移位推进，禁止复制权重表或退化为分档/运行时除法。重复 finish/checkpoint 必须 fail-stop，snapshot 不得写回或累加 live Thread 私有值，否则同一时间片会被重复观察，且 exited Thread 的 Process runtime 会丢失。
