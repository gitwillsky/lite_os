...
use user_lib::{
    TestStats, chdir, chmod, chown, close, dup, dup2, errno, exit, fcntl_consts, fcntl_getfl,
    fcntl_setfl, fd_flags, flock, flock_consts, getcwd, listdir, lseek, mkdir, mkfifo, open,
    open_flags, pipe, read, remove, stat, test_section, test_subsection, write,
};
...
fn test_directory_operations(stats: &mut TestStats) {
    ...
    test_assert!(
        cwd_str.ends_with("/tmp_fs_test") || cwd_str == "/tmp_fs_test",
        "cwd 不正确: {}",
        cwd_str
    );
...
fn test_file_basic_operations(stats: &mut TestStats) {
    ...
    test_assert!(
        write_len as usize == test_data.len(),
        "写入长度不匹配: {} != {}",
        write_len,
        test_data.len()
    );
    ...
    test_assert!(
        read_len as usize == test_data.len(),
        "读取长度不匹配: {} != {}",
        read_len,
        test_data.len()
    );
