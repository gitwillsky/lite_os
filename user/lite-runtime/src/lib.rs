//! Generic LiteOS GUI/JS runtime library: one process, one QuickJS VM, one
//! React root and one top-level surface. Each app binary links this crate and
//! calls [`run`], passing any app-specific native capabilities as
//! [`HostExtension`]s. App-specific ops (desktop shell, terminal PTY, music
//! provider/network) do NOT live here.
...
pub use host::{Action, ExtensionCx, ExtensionEvents, HostExtension};
pub use quickjs_runtime::{EngineError, Role};
// Streaming-download infrastructure exposed for app extensions that
// download-while-playing (e.g. the music player registers a `SharedStream`
// through `ExtensionCx::register_stream`).
pub use audio::{SharedStream, StreamState};
...
/// Whether this process hosts the desktop shell or a single application.
enum Mode {
    Desktop,
    App(String),
}
...
/// Runs one LiteOS GUI process to completion.
///
/// `role` selects desktop vs. application behavior; `app_id` names the bundle
/// under `/usr/share/liteos` to load. `extensions` are app-provided native
/// capabilities layered onto the generic host (see [`HostExtension`]).
pub fn run(
    role: Role,
    app_id: &str,
    ...
    let mode = match role {
        Role::Desktop => Mode::Desktop,
        Role::App => Mode::App(app_id.to_owned()),
    };
    let root = match &mode {
        Mode::Desktop => PathBuf::from("/usr/share/liteos/desktop"),
        Mode::App(id) => PathBuf::from("/usr/share/liteos/apps").join(id),
    };
    let runtime = fs::read("/usr/lib/lite-runtime/runtime.js")
    ...
    loop {
        ...
        // Drain any app-extension worker events (e.g. a music download's
        // progress) and dispatch them to their JS channel. Empty for apps with
        // no extensions.
        if extension_ready {
            for (channel, value) in state.drain_extension_events() {
                dispatch(&mut engine, &channel, value)
                    .map_err(|error| owner_error("extension event dispatch", error))?;
            }
            engine
                .run_jobs()
...
fn process_actions(
    ...
    for action in state.take_actions() {
        match action {
            Action::Launch(id) => {
                // Each app is now its own binary (`/bin/<id>`) that links this
                // runtime library, rather than one shared `/bin/lite-ui --app`.
...
fn wait(
    ...
    // App extensions publish worker events through the shared State and wake the
    // loop over one edge-triggered fd; poll it when any extension registered one.
    let extension_index = descriptors.len();
    let extension_fd = state.extension_event_fd();
    ...
    Ok((
        ...
        extension_fd.is_some() && descriptors[extension_index].returned() != PollEvents::EMPTY,
    ))
}
