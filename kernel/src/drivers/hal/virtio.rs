...
impl VirtIODevice {
    pub fn new(base_addr: usize, size: usize) -> Result<Self, DeviceError> {
        ...
        let device_id = bus
            .read_u32(VIRTIO_MMIO_DEVICE_ID)
            .map_err(DeviceError::from)?;
        let vendor_id = bus
            .read_u32(VIRTIO_MMIO_VENDOR_ID)
            .map_err(DeviceError::from)?;
    ...
    pub fn probe_virtio(&self) -> Result<bool, DeviceError> {
        ...
        let magic = bus
            .read_u32(VIRTIO_MMIO_MAGIC_VALUE)
            .map_err(DeviceError::from)?;
        let version = bus
            .read_u32(VIRTIO_MMIO_VERSION)
            .map_err(DeviceError::from)?;
    ...
    pub fn device_features(&self) -> Result<u32, DeviceError> {
        let bus = self.inner.bus();
        bus.read_u32(VIRTIO_MMIO_DEVICE_FEATURES)
            .map_err(DeviceError::from)
    }

    pub fn set_driver_features(&self, features: u32) -> Result<(), DeviceError> {
        let bus = self.inner.bus();
        bus.write_u32(VIRTIO_MMIO_DRIVER_FEATURES, features)
            .map_err(DeviceError::from)
    }

    pub fn set_status(&self, status: u32) -> Result<(), DeviceError> {
        let bus = self.inner.bus();
        bus.write_u32(VIRTIO_MMIO_STATUS, status)
            .map_err(DeviceError::from)
    }
...
impl Device for VirtIODevice {
    ...
    fn request_resources(
        &mut self,
        resource_manager: &mut dyn super::resource::ResourceManager,
    ) -> Result<(), DeviceError> {
