...
| ext/JBD2 on-disk protocol | Linux `v7.1` 同一固定 commit；`Documentation/filesystems/ext4/journal.rst`、`include/linux/jbd2.h`、`fs/jbd2/{transaction,commit,recovery}.c` | [JBD2 documentation](https://github.com/torvalds/linux/blob/8cd9520d35a6c38db6567e97dd93b1f11f185dc6/Documentation/filesystems/ext4/journal.rst)、[fixed transaction implementation](https://github.com/torvalds/linux/blob/8cd9520d35a6c38db6567e97dd93b1f11f185dc6/fs/jbd2/transaction.c)、[fixed implementation tree](https://github.com/torvalds/linux/tree/8cd9520d35a6c38db6567e97dd93b1f11f185dc6/fs/jbd2) | journal superblock/header/tag/commit 的 big-endian layout、escape、flush/commit/replay 顺序、buffer-credit capacity/restart 与 ext orphan recovery；writeback 不得把每页强制扩大为一次完整 commit |
...
| Display font | JetBrains Mono `2.304`；官方 zip SHA-256 `6f6376c6ed2960ea8a963cd7387ec9d76e3f629125bc33d1fdcd7eb7012f7bbf`；`JetBrainsMonoNL-{Medium,Bold}.ttf` SHA-256 分别为 `44099e1efefba55637e0abbbf8dd3f526e59523345888a257bb01d39df4af74c`、`0198e841824025f8876e5c297f0b9b497ee8d6eb9969710a3328e1303f996ec3`；SIL OFL 1.1 | [官方 release](https://github.com/JetBrains/JetBrainsMono/releases/tag/v2.304)、[固定 tag](https://github.com/JetBrains/JetBrainsMono/tree/v2.304) | 显式 generator 的唯一字体 source；产出固定 464 glyph、16×32 Medium/Bold A8 atlas（SHA-256 `955331fdc5fd5368e9cfe8771154abf4c4d40fe0db961d2df28859d06fa4b677`），v2 header 唯一定义 session-invariant cell metrics；普通构建只嵌入 atlas，TTF 不进入 rootfs |
| seatd / libseat | 官方 `0.9.3`；tag object `24dd75b462660e2244a023d637a6c753049e0d9d`；peeled commit `daa8196e10b180b8b0caeafa8e5f860eb1bd6706` | [固定源码](https://git.sr.ht/~kennylevinsen/seatd/tree/daa8196e10b180b8b0caeafa8e5f860eb1bd6706) | `/run/seatd.sock` 上 `proto_header`、client/server opcode、payload、SCM_RIGHTS 与 libseat callback/ACK 顺序；LiteOS broker 只实现该 wire，不复制 upstream VT/device-state owner |
| libdrm | 官方 `2.4.134`；tag object `b42a9d939c896ef9b1ef9423218fb9668d616d93`；peeled commit `e984d448b8b17aab853369e6c203e53719f46de1` | [固定源码](https://gitlab.freedesktop.org/mesa/drm/-/tree/e984d448b8b17aab853369e6c203e53719f46de1) | 唯一 compositor/display-session client 的 userspace DRM UAPI codec；terminal service 不链接 libdrm；不定义 kernel KMS 语义，也不得形成 raw-ioctl 双轨 |
| QuickJS | 官方 `2026-06-04`；release tarball SHA-256 `b376e839b322978313d929fd20663b11ba58b75df5a46c126dd19ea2fa70ad2a` | [官方 release](https://bellard.org/quickjs/quickjs-2026-06-04.tar.xz)、[官方文档](https://bellard.org/quickjs/quickjs.html) | 每个 `liteui-host` 内唯一 ECMAScript Runtime；只执行签名 APK 的 `app.mjs` 与派生 bytecode cache，不定义 UI tree、layout、frame clock、window policy 或 OS capability |
| SolidJS universal renderer | npm `solid-js@1.9.14`；archive SHA-256 `0aae69da57139bb29fc6f4b0055a8a1804e8eafbef26dd1099c3039fd5f7e0f5` | [npm package](https://www.npmjs.com/package/solid-js/v/1.9.14)、[upstream repository](https://github.com/solidjs/solid) | host build 将官方 core 与 universal renderer 静态并入 source-authoritative `app.mjs`；guest 无 npm/DOM，Rust adapter 是唯一 host renderer seam |
| Taffy | 官方 crate `0.12.2`；crate SHA-256 `340a09581f29809fc0df82a3955501dc7f2a21f887e5d1c13dbe288fe1c0bef4`；禁用 default features，仅启用 `alloc/taffy_tree/flexbox/content_size` | [官方 crate 文档](https://docs.rs/taffy/0.12.2/taffy/)、[官方仓库](https://github.com/DioxusLabs/taffy) | `liteui-core` 内 Flexbox layout mechanism；不拥有 retained node identity、transaction publication、style ABI、text、window geometry 或 allocation/OOM policy |
...
fd allocation 额外固定同 revision 的 [`fs/file.c`](https://github.com/torvalds/linux/blob/8cd9520d35a6c38db6567e97dd93b1f11f185dc6/fs/file.c)：`alloc_fd(start,end)` 必须返回 `[start,end)` 内最低空闲 fd，close 使该 slot 可复用；`next_fd`、open bitmap 与 full-word summary 只是同一 files owner 下的搜索结构。`receive_fd()` 固定 reserve fd → copyout fd number → publish file 的顺序，因此当前 copyout 失败不得泄漏可见但未告知 userspace 的 descriptor，也不得回滚已经发布的前缀。LiteOS 可采用更深但固定有界的 summary hierarchy，不得改变 minimum/limit/OOM/visible-publication 语义，或把索引复制到 syscall/Process 形成第二 owner。

AF_UNIX descriptor passing 额外固定同 revision 的 [`include/net/scm.h`](https://github.com/torvalds/linux/blob/8cd9520d35a6c38db6567e97dd93b1f11f185dc6/include/net/scm.h)、[`net/core/scm.c`](https://github.com/torvalds/linux/blob/8cd9520d35a6c38db6567e97dd93b1f11f185dc6/net/core/scm.c)、[`net/unix/af_unix.c`](https://github.com/torvalds/linux/blob/8cd9520d35a6c38db6567e97dd93b1f11f185dc6/net/unix/af_unix.c) 与 [`net/unix/garbage.c`](https://github.com/torvalds/linux/blob/8cd9520d35a6c38db6567e97dd93b1f11f185dc6/net/unix/garbage.c)：它们共同固定 `SCM_MAX_FD`、control truncation、real-UID inflight accounting、`ETOOMANYREFS` 与 source→receiver graph/cycle collection。LiteOS 在发送 commit 路径同步执行 iterative Tarjan，而 Linux 当前可由 worker 调度 collection；这是内部调度差异，不能改变边、root、inflight 或用户可见错误语义。
...
DRM/KMS 与 hotplug ABI 额外固定同 revision 的 [`include/uapi/drm/drm_mode.h`](https://github.com/torvalds/linux/blob/8cd9520d35a6c38db6567e97dd93b1f11f185dc6/include/uapi/drm/drm_mode.h)、[`drivers/gpu/drm/drm_framebuffer.c`](https://github.com/torvalds/linux/blob/8cd9520d35a6c38db6567e97dd93b1f11f185dc6/drivers/gpu/drm/drm_framebuffer.c)、[`include/uapi/linux/netlink.h`](https://github.com/torvalds/linux/blob/8cd9520d35a6c38db6567e97dd93b1f11f185dc6/include/uapi/linux/netlink.h) 与 [`lib/kobject_uevent.c`](https://github.com/torvalds/linux/blob/8cd9520d35a6c38db6567e97dd93b1f11f185dc6/lib/kobject_uevent.c)：`drm_mode_fb_dirty_cmd` 固定 24-byte layout、flags mask、copy-pair rule、pointer/count relation 与 clip copyin，driver 可扩大、延迟或合并 damage；LiteOS 精确声明单次 32 clips 的较小实现上限。connector hotplug 使用 AF_NETLINK/NETLINK_KOBJECT_UEVENT 的 NUL-separated environment message 与 multicast group，不建立私有 display event ABI。
...
scheduler runnable ownership 额外固定同 revision 的 [`kernel/sched/core.c`](https://github.com/torvalds/linux/blob/8cd9520d35a6c38db6567e97dd93b1f11f185dc6/kernel/sched/core.c) 与 [`kernel/sched/fair.c`](https://github.com/torvalds/linux/blob/8cd9520d35a6c38db6567e97dd93b1f11f185dc6/kernel/sched/fair.c)：enqueue/dequeue 必须与 authoritative runnable state/accounting 属于同一 runqueue transaction，fair ordered structure 增量维护。LiteOS 的 generation token 可以延迟回收 physical stale entry，但 logical Ready projection 必须随 SchedulingState 转换提交，且 ordinary enqueue/delivery 不得以全容器 validation 冒充增量 runqueue mutation。
