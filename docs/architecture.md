...
BusyBox 官方 release 1.37.0 已固定 tarball SHA-256 与唯一 `init + ash`/基础 applet config，并能通过固定 musl sysroot 构建为静态 RISC-V `ET_EXEC`。当前只把 source/config/ELF 构建作为 gate；产物尚未进入 rootfs，也未作为 kernel runtime 通过，因此当前仍明确没有 shell。
...
- 固定 BusyBox 1.37.0 官方 tarball/config/musl sysroot，检查静态 RISC-V `ET_EXEC`、offset-zero LOAD、W^X 与 non-executable stack；该静态 gate 不替代 runtime 启动证据。
