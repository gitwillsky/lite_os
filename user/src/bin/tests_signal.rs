...
use user_lib::{
    SIG_BLOCK, SIG_DFL, SIG_IGN, SIG_SETMASK, SIG_UNBLOCK, SigAction, TestStats, alarm, exit, fork,
    getpid, kill, pause, sigaction, signal, signals, sigprocmask, sigreturn, sleep_ms,
    test_section, test_subsection, wait_pid,
};
...
fn test_basic_signal_handling(stats: &mut TestStats) {
    ...
    test_assert!(
        LAST_SIGNAL.load(Ordering::SeqCst) == signals::SIGUSR1 as i32,
        "最后信号记录错误"
    );
...
fn test_multiple_signal_handlers(stats: &mut TestStats) {
    ...
    test_assert!(
        sigaction(signals::SIGUSR1, &act1, core::ptr::null_mut()) == 0,
        "安装SIGUSR1处理器失败"
    );
    test_assert!(
        sigaction(signals::SIGUSR2, &act2, core::ptr::null_mut()) == 0,
        "安装SIGUSR2处理器失败"
    );
