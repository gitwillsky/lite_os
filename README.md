...
- Sv39、统一 VMA owner、独立用户地址空间、页级 W^X、栈 guard page、`brk`，以及 eager private、lazy regular-file shared `mmap/munmap/mprotect/msync` 与 `MAP_FIXED`。
...
- 带标准 JBD2 metadata journal 的读写 ext2 revision 1 启动卷与 boot-time device filesystem mount；进程通过统一 fd/OFD 模型访问 regular file、directory、symlink、pipe 与 `/dev/null|zero|tty|console`。
...
- 106 个 Linux/riscv64 syscall number；credentials/permission、UART/TTY、Pipe、pselect/ppoll/epoll、AF_UNIX、AF_INET UDP/TCP/raw ICMP、标准链接、共享文件映射、getrandom、sysinfo/statfs、uname/realtime、RISC-V hwprobe 与 system reset 统一经过各自的单一 owner，未实现编号返回 `-ENOSYS`。

LiteOS 当前不支持 futex PI/requeue/bitset、完整 clone/vfork flags、所有 syscall 的 restart、signal altstack/realtime queue、pathname AF_UNIX、IPv6/raw socket、anonymous shared mapping、后台 page-cache reclaim/writeback worker、完整 pthread runtime、GUI 或任意 musl 程序。动态链路证明固定 BusyBox（含基础 job control）、`dlopen` consumer 与 AF_INET UDP/TCP 竖切，不外推为完整 Linux、POSIX.1-2024 或 musl conformance。
...
当前工具集除 `init + ash` 与基础文件命令外，还包含 `vi/less/more`、`diff/patch/cmp`、`hexdump/hd/od/strings`、`awk/sed`、`head/tail/cut/sort/uniq/tr/tee`、`find/xargs`、`basename/dirname/readlink/realpath/which/env/expr/seq/sleep`、`tar/unzip`、`gzip/bzip2`、XZ 解压与 `sha256sum`。每个入口都由 BusyBox UART gate 执行真实文件、终端、pipe、差异应用、归档、压缩或校验路径，不以“编译进 ELF”代替运行支持。BusyBox `xz` 上游 applet 仅提供解压，不声明 XZ 压缩能力。
