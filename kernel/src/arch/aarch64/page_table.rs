use super::{
    mmu::{
        self, AddressSpaceToken, KernelTrapToken, allocate_address_space_id,
        release_address_space_id_after_global_fence,
    },
    pte::{self, PagePermissions},
    va39,
};
use crate::fallible_tree::{FallibleMap, VacantEntry};

const PAGE_SHIFT: usize = 12;
const VA39_LEVELS: usize = 3;

/// Stage-1 root role；kernel root belongs to TTBR1 and does not consume a user ASID。
...
impl<Page> RetiredTablePages<Page> {
    ...
    fn push(&mut self, entry: VacantEntry<usize, Page>) {
        let slot = self
            ...
            .expect("AArch64 39-bit unmap retired more than two table levels");
        *slot = Some(entry);
    }
...
impl core::fmt::Display for PageTableError {
    fn fmt(&self, formatter: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::InvalidFlags => write!(formatter, "invalid AArch64 PTE attributes"),
            Self::InvalidPageTable => {
                write!(formatter, "invalid AArch64 39-bit page-table structure")
            }
            Self::AddressSpaceIdentifiersExhausted => {
                write!(formatter, "AArch64 address-space identifiers exhausted")
            }
        }
    }
}
...
impl PageTableEntry {
    fn new(physical_page: usize, attributes: u64, level: usize) -> Self {
        let address = (physical_page as u64) << PAGE_SHIFT;
        let descriptor_type = if level == 2 { pte::TABLE_OR_PAGE } else { 0 };
        Self((address | attributes | descriptor_type) as usize)
    }

    fn table(physical_page: usize) -> Self {
        Self((((physical_page as u64) << PAGE_SHIFT) | pte::VALID | pte::TABLE_OR_PAGE) as usize)
    }
    ...
    pub(crate) fn permissions(self) -> PagePermissions {
        pte::decode(self.0 as u64)
    }

    pub(crate) fn physical_page(self) -> usize {
        (self.0 >> PAGE_SHIFT) & ((1usize << 36) - 1)
    }

    fn is_valid(self) -> bool {
        self.0 as u64 & pte::VALID != 0
    }

    fn is_leaf_at(self, level: usize) -> bool {
        self.is_valid() && (level == 2 || self.0 as u64 & pte::TABLE_OR_PAGE == 0)
    }

    fn is_next_table_at(self, level: usize) -> bool {
        level < 2 && self.is_valid() && self.0 as u64 & pte::TABLE_OR_PAGE != 0
    }
}
...
impl<Page: TablePage> PageTable<Page> {
    pub(crate) fn try_new(kind: AddressSpaceKind) -> Result<Self, PageTableError> {
        ...
        let address_space_id = match kind {
            AddressSpaceKind::Kernel => 0,
            AddressSpaceKind::User => allocate_address_space_id()
                .ok_or(PageTableError::AddressSpaceIdentifiersExhausted)?,
        };
        Ok(Self {
            root_page,
            table_pages,
    ...
    fn read_entry(table_page: usize, index: usize) -> PageTableEntry {
        assert!(index < 512, "AArch64 39-bit table index exceeds one page");
        let pointer = mmu::physical_to_virtual(table_page << PAGE_SHIFT) as *const PageTableEntry;
    ...
    fn write_entry(table_page: usize, index: usize, entry: PageTableEntry) {
        assert!(index < 512, "AArch64 39-bit table index exceeds one page");
        let pointer = mmu::physical_to_virtual(table_page << PAGE_SHIFT) as *mut PageTableEntry;
    ...
    fn map_leaf(
        ...
        Self::write_entry(
            table,
            index,
            PageTableEntry::new(physical_page, flags, level),
        );
        Ok(())
    }
    ...
    pub(crate) fn unmap(&mut self, virtual_page: usize) -> Result<Unmapped<Page>, PageTableError> {
        ...
        for level in 0..VA39_LEVELS {
            ...
            if level == 2 || !entry.is_next_table_at(level) {
                return Err(PageTableError::InvalidPageTable);
            }
    ...
    pub(crate) fn set_flags(
        ...
        for (level, index) in indexes[..2].iter().enumerate() {
        ...
        Self::write_entry(
            table,
            index,
            PageTableEntry::new(old.physical_page(), flags, 2),
        );
        Ok(())
    }
        ...
        for (level, index) in indexes.into_iter().enumerate() {
            ...
            if entry.is_leaf_at(level) {
                ...
                return Some(PageTableEntry::new(
                    ...
                    entry.0 as u64 & !((((1u64 << 36) - 1) << PAGE_SHIFT) | pte::TABLE_OR_PAGE),
                    2,
                ));
            }
            if !entry.is_next_table_at(level) {
