...
impl Host {
    ...
    fn load_media(&self, payload: &str) -> Result<String, EngineError> {
        ...
        // A `stream:<id>` src plays a still-downloading online track; resolve it
        // to its temp path + shared download state so the decoder streams it.
        if let Some(rest) = request.src.strip_prefix("stream:") {
            let stream_id: u64 = rest
                .trim()
                .parse()
            ...
            let (path, state) = self
                ...
                .ok_or_else(|| EngineError::from_host("unknown stream id"))?;
