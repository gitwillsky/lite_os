...
- 静态 ELF64 `ET_EXEC`，Linux 形式 `argc/argv/envp/auxv` 初始栈；固定 musl v1.2.6 pthread create/join、mutex/condition 与 timedwait consumer 已纳入冷启动围栏。
- 39 个 Linux/riscv64 syscall number；文件入口统一经过 fd/OFD 与 VFS，未实现编号统一返回 `-ENOSYS`。

LiteOS 当前不支持 futex PI/requeue/bitset 与 signal interruption、完整 clone/vfork flags、signal restart/altstack、pipe/socket/IPC、file/shared/lazy mapping、PIE、动态链接、完整 pthread runtime、shell、GUI、network userspace 或常规 musl 程序。固定 pthread consumer 通过不外推为通用 musl 兼容。项目不声称完整 Linux 兼容、POSIX.1-2024 符合或 musl 兼容。
...
该目标校验官方 release tarball SHA-256，在 `target/musl-static/` 构建一次性工具链产物，将 `user/musl-smoke.c` 链接为静态 `ET_EXEC`，再以独立 ext2 镜像冷启动并要求输出 `LiteOS musl pthread sync ok`。consumer 实际执行 `pthread_create/join`、mutex/condition 双向同步与 `pthread_cond_timedwait` 超时；musl 源码和二进制不进入仓库。
