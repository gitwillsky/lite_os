...
- ext2 `Inode` 只暴露 `StorageWriter` batch seam，由 page cache 决定 coherence、固定 32-page scan batch、capacity-pressure split 与 committed-prefix clean publication；ext2 journal 继续唯一拥有 metadata/data 提交顺序与 transaction capacity。
...
- fitting dirty batch 共享一次 ext2 transaction并只在 commit 后清 clean；journal capacity 不足时二分为有界完整 transaction，任一失败保留未提交 suffix dirty，单页失败透传原 errno。
