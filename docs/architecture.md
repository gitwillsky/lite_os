...
U-mode 使用 Linux/riscv64 约定：`a7=number`、`a0..a5=args`、`a0=result`、kernel error 为负 errno。共享 crate 只有 138 个已接入的 Linux number，未识别 number 一律安静地返回 `-ENOSYS`；用户可控的正常 capability/io_uring probe 不写 kernel log。
