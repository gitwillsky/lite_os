...
BusyBox 官方 release 1.37.0 已固定 tarball SHA-256、唯一 `init + ash`/基础 applet config 与 inittab。默认 `make build` 构造并静态检查单 BusyBox inode/hardlink rootfs，`make run` 直接启动该镜像；仓库不再存在 Rust init/runtime 或另一条 rootfs 路径。gate 验证 ash 算术、pipeline、重定向、后台 wait、可执行 script、process-group kill、`ps/free/uptime/top` 系统可观测性、跨冷启动持久化、foreground Ctrl-Z/jobs/bg/fg/Ctrl-C，以及 `stty` 驱动的后台 read/SIGTTIN 与 `TOSTOP` write/SIGTTOU 停止-恢复链。当前不证明未选中 applet。

## 13. Syscall ABI

U-mode 使用 Linux/riscv64 约定：`a7=number`、`a0..a5=args`、`a0=result`、kernel error 为负 errno。共享 crate 只有 63 个已实现的 Linux number，未识别 number 一律 `-ENOSYS`。
...
1. **文件系统容量可观测性**：按 Linux `statfs/fstatfs` ABI 接通 VFS/filesystem 唯一容量 owner，并以 BusyBox `df` 形成下一条可验证竖切。
