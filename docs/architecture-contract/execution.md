...
- 每个 Thread 的 `ContextOwner<UserContext>` 独占 trap-context VA、稳定 physical pointer 与
  mutable transaction capability；AddressSpace 只在 create/exec/retire seam bind、rebind 或 unmap。
...
- boot CPU 必须在设备初始化前建立 membarrier per-CPU state；否则启动期 SSIP handler
  ...
  SEIE/SSIE/SIE 并执行 architecture-owned WFI。hardirq 发布的 pending SSIP 是已确认
  device edge 的耐久 wake token，但领域 deferred consumer 仍只能在 user-return/idle safe point
  执行。WFI 具有唯一 linked
  PC/resume identity；external/software trap 若在 enable 后、WFI 前被确认，kernel trap entry 把
  `sepc` 推进到 resume PC，禁止返回后消费已经不存在的 edge 而永久睡眠。不使用
  flag、MMIO poll 或第二个 IRQ owner。返回前精确恢复原 SEIE/SSIE/SIE，并保持
  timer source 不变，直到既有 scheduler enable seam 完成其 owner 初始化。
