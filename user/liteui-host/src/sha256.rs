...
pub fn digest(input: &[u8]) -> [u8; 32] {
    ...
    let final_length = if remainder.len() < 56 { 64 } else { 128 };
    let bits = (input.len() as u64).wrapping_mul(8).to_be_bytes();
    final_blocks[final_length - 8..final_length].copy_from_slice(&bits);
...
fn compress(state: &mut [u32; 8], block: &[u8]) {
    ...
    for (index, bytes) in block.chunks_exact(4).enumerate() {
        words[index] = u32::from_be_bytes(bytes.try_into().unwrap_or([0; 4]));
    }
    for index in 16..64 {
