...
pub fn decode_png_rgba(data: &[u8]) -> Option<(u32,u32,Vec<u8>)> {
    ...
    let mut i = 8usize; let mut width = 0u32; let mut height = 0u32; let mut idat: Vec<u8> = Vec::new(); let mut color_type = 6u8; let mut bit_depth = 8u8; let mut interlace = 0u8;
    while i + 8 <= data.len() {
        ...
        match typ {
            b"IHDR" => {
                ...
                bit_depth = payload[8]; color_type = payload[9]; interlace = payload[12];
                if bit_depth != 8 || !(color_type==6 || color_type==2) || interlace!=0 { return None; }
            }
