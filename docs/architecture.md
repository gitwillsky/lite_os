...
TaskManager 的唯一 indexed wait registry 拥有 wait registration，并可同时将它索引到 `(TGID,uaddr)` 和 absolute deadline。队列锁覆盖 futex 用户值比较、`rt_sigtimedwait` pending 复查与 `WaitMembership` 发布；wake、timeout 和 signal interruption 都原子删除同一 registration 及全部索引。blocking path 在 owner lock 内复查 deliverable/matching signal，因此不存在 compare/enqueue、signal/enqueue lost wakeup 或双重消费。当前支持 futex WAIT/WAKE、PRIVATE flag、WAIT relative monotonic timeout、thread-directed signal interruption，以及 signal-set wait 的可选 deadline；不支持 futex PI、requeue 或 bitset。
...
U-mode 使用 Linux/riscv64 约定：`a7=number`、`a0..a5=args`、`a0=result`、kernel error 为负 errno。共享 crate 只有 42 个已实现的 Linux number，未识别 number 一律 `-ENOSYS`。
