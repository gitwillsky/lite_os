...
- 首个 graphics consumer 是紧凑常驻 Rust 2D reference client：libseat+libdrm、两个 dumb buffer、geometry page-flip event、固定容量 DIRTYFB pointer damage、software geometry/pointer、keyboard/mouse 与 resize/hotplug。main reactor 唯一拥有 Display/Scene，固定 presenter pthread 通过单槽 SPSC+eventfd 执行 blocking DIRTYFB；input/seat/hotplug 不得等待 GPU，worker 在途时不得以 render/resize deadline 轮询。pointer report 只传输旧、新光标矩形；geometry flip 先同步 inactive buffer，再复用两槽 resident GPU resource 做 SET_SCANOUT→FLUSH，禁止逐帧 CREATE/ATTACH/UNREF；revoke/exit 先 drain request 再释放 framebuffer/join，idle 时两个线程都阻塞；render 上限 60 fps，不引入 toolkit、compositor、Mesa/GBM/EGL、字体、图片或 3D。
