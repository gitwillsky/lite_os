...
| 20-22 | `epoll_create1/epoll_ctl/epoll_pwait` | Linux RV64 epoll event ABI | ready count；标准 errno | Linux epoll；musl event loop | **Partial**。当前 pollable OFD 支持 LT/ET/ONESHOT、PRI/RDHUP、5 层无环嵌套、ready-list 轮转、timeout/temporary signal mask、跨 fork 最后 descriptor close、`(fd,OFD)` reuse identity 与并发 close/MOD/delivery revision；单次 copyout 失败不消费事件。无 suspend policy 时按 Linux 非特权语义忽略 `EPOLLWAKEUP`；尚无需要 wake-one registration 的 `EPOLLEXCLUSIVE`，其余当前无 source 的 poll 扩展位不开放。 | `kernel/src/syscall/epoll.rs`, `kernel/src/fs/epoll.rs`, `kernel/src/syscall/poll.rs` |
