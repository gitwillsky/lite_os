...
- 每个 live Thread 都拥有固定 kernel stack，因此 topology 用物理页容量/kernel-stack 页数证明 runnable membership 上界，并在启动时为每 hart local heap 与 inbound mailbox 一次预留；runtime 入队前原地清除 stale generation，mailbox drain 每轮只取一次锁、对 mailbox/local heap 各扫描一次并批量转移，同时保留 backing capacity，wake/IRQ 路径不分配。
