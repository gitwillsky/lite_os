...
Signal disposition 与 coalesced process-directed pending bit+首个 siginfo 由共享 Process 的同一 `ProcessSignalState` lock 拥有；mask、thread-directed pending 与至多一条 syscall replay record属于 Thread。`tgkill` 发布 SI_TKILL 到 Thread；`kill` 按 TGID/caller-PGID/all-except-init-and-caller/explicit-PGID selector 发布 SI_USER 到 Process shared pending；TTY group signal 与 SIGCHLD/CLD_EXITED 复用同一路径。delivery 与 `rt_sigtimedwait` 都先消费 Thread pending、再消费 shared pending；发布后只唤醒一个 matching signal waiter 或未屏蔽的 eligible Thread。未屏蔽、非忽略 signal 会取消 deadline/futex/child/console/signal wait 并以 `Interrupted` 恢复。trap/syscall 边界把可重启结果暂存为 userspace `EINTR`，同时保存原 `a0..a5/a7/ecall PC`；实际 handler 含 `SA_RESTART` 时 frame 才保存 replay context。trap return 构造 Linux RV64 `siginfo + ucontext + sigcontext` 并保存完整 GPR/FP state；`rt_sigreturn` 恢复 mask、寄存器、FP 与 PC。当前 blocking `wait4` 和无 timeout futex WAIT 可重启；带 relative timeout futex 与 `nanosleep` 保持 `EINTR`。无 altstack、queued realtime value、credential permission check 或 stop/continue scheduler lifecycle。
...
BusyBox 官方 release 1.37.0 已固定 tarball SHA-256、唯一 `init + ash`/基础 applet config 与 inittab。默认 `make build` 构造并静态检查单 BusyBox inode/hardlink rootfs，`make run` 直接启动该镜像；仓库不再存在 Rust init/runtime 或另一条 rootfs 路径。gate 验证 ash 算术、外部 applet pipeline、文件重定向、后台 subshell+wait、foreground Ctrl-C、可执行 shebang script、kill signal-zero/process-group TERM 和跨冷启动持久化。当前不证明 stop/continue job control、全部 termios action 或未选中 applet。

## 13. Syscall ABI

U-mode 使用 Linux/riscv64 约定：`a7=number`、`a0..a5=args`、`a0=result`、kernel error 为负 errno。共享 crate 只有 61 个已实现的 Linux number，未识别 number 一律 `-ENOSYS`。
...
- vDSO 与 `AT_HWCAP`。
- network/GPU/input user device ABI，以及完整 VMIN/VTIME 与 stop/continue 语义。
...
1. **job-control 完整化**：以 BusyBox ash 实际调链完成 process-directed/group signal、stop/continue lifecycle 和剩余 Terminal action。
