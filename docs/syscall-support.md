...
| 93 | `exit` | `int status` | 不返回 | POSIX `_exit`；musl `_Exit` fallback | **Complete**。只终止 calling Thread；最后一个 Thread 才以低 8-bit code 发布 Process zombie。 | `kernel/src/syscall/process.rs`, `kernel/src/task/task_manager/process_exit.rs` |
| 94 | `exit_group` | `int status` | 不返回 | Linux extension；musl `_Exit` 首选 | **Complete**。首个 caller 唯一提交低 8-bit group status，解除全部 sibling wait 并请求远端 CPU 调度；每个 Thread 在自己的内核栈执行 robust/clear-tid/调度清理，最后一个 Thread 发布 zombie。 | `kernel/src/syscall/process.rs`, `kernel/src/task/task_manager/process_exit.rs` |
...
| 129 | `kill` | signed pid selector、signal | 0；`ESRCH/EINVAL` | POSIX kill；BusyBox kill/job control | **Partial**。支持 TGID、caller PGID、`-1` 排除 init/caller、negative PGID、signal-zero probe、SI_USER shared pending、stop/continue conflict elimination、group stop/resume 与多线程默认致命 action；固定 root identity 无 permission failure，尚无 PID 1 unkillable policy。 | `kernel/src/syscall/signal.rs`, `kernel/src/task/task_manager/signal.rs`, `kernel/src/task/model/signal_state.rs` |
