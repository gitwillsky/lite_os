...
- rootfs 只有一个 BusyBox ELF inode；`/bin/init`、`/bin/sh` 与选定基础 applet 全部是指向它的 hardlink，`/etc/inittab` 同时监督 UART ash、固定 musl PIE `/bin/liteos-terminal` 与一次性 `/bin/display-session-guard`，shell 可显式启动固定 musl PIE `/bin/liteos-2d`。完整 userspace 由本地 RSA256 签名的 `liteos-base` APK 唯一拥有，最终 `/lib/apk/db/installed` 只能由 guest 内真实 `apk.static --initdb add` 生成；host builder 不伪造 package database，也不保留未登记的第二套 runtime。
...
- `/etc/inittab` 先以独立 `once` action 启动 display-session guard，再以独立 `respawn` action 启动 terminal、前台 `udhcpc` 与 UART login-style root ash，不保留 `askfirst`/Enter 激活入口。broker 任意退出都由 guard 执行 sync+cold reboot，避免在设备撤销状态不可证明时建立第二 owner；普通 terminal/shell/service exit 仍由 init 局部恢复。`make run-gui` 默认只显示 1920×1080 的窗口化 Cocoa Display，关闭 QEMU monitor，并把 UART 保留到 `target/run-gui-serial.log`；`make run` 仍是显式 UART 开发入口。`run/run-gui/run-gdb` 共用显式 512 MiB 客体内存基线，kernel 仍只消费 DTB 报告的动态物理内存范围。network service 的 stdout/stderr 截断写入 `/run/network-service.log`，后台 lease/renew 日志不与交互命令行竞争 UART。
...
4. **显示与输入 I/O**：modern VirtIO-GPU 2D、精确 SG backing、IRQ-driven scanout/damage/disable、标准 `/dev/dri/card0`、DIRTYFB、kobject hotplug、GEM/dumb mmap、master 与 page-flip event 已闭环；VirtIO keyboard→evdev→Unix98 PTY/devpts→ash→Rust terminal→DRM 的无轮询竖切也已闭环。[Phase 60 display-session](phase-60-display-session.md) 所定义的 DRM/input capability handoff、失效安全撤销、terminal 冷重建与首个 double-buffered 2D reference consumer 已实现；下一步以 terminal↔2D 往返和 resize 运行时测量验证 graphics client seam，禁止暴露 VirtIO 私有 ioctl、CPU 全帧 memcpy、永久 fallback 或恢复运行期自旋。
