...
impl DmaPool {
    pub fn new(
        ...
        use crate::memory::{PAGE_SIZE, frame_allocator::alloc_contiguous};
...
pub trait DmaBuffer: Send + Sync {
    ...
    fn split_at(
        &self,
        offset: usize,
    ) -> Result<(Box<dyn DmaBuffer>, Box<dyn DmaBuffer>), MemoryError>;
}
...
pub trait DmaManager: Send + Sync {
    fn allocate_buffer(
        ...
        size: usize,
        coherent: bool,
    ) -> Result<Box<dyn DmaBuffer>, MemoryError>;
    fn allocate_buffer_with_attributes(
        &self,
        size: usize,
        coherent: bool,
        attributes: MemoryAttributes,
    ...
    fn create_pool(
        ...
        allocation_size: usize,
        coherent: bool,
    ) -> Result<Arc<DmaPool>, MemoryError>;
...
impl CoherentDmaManager {
    ...
    fn allocate_virtual_address(&self, size: usize, align: usize) -> Result<usize, MemoryError> {
        ...
        loop {
            ...
            if self
                .next_virt_addr
                .compare_exchange_weak(current, new_addr, Ordering::Relaxed, Ordering::Relaxed)
                .is_ok()
            {
