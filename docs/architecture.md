...
- rootfs 固定的 `/bin/liteos-terminal` 是唯一显示 terminal consumer：查询标准 DRM/KMS topology，建立一个 XRGB8888 dumb framebuffer，以 page-flip completion 驱动刷新；扫描 keyboard evdev，使用 PTY 启动新的 login-style ash。固定 16×32 cell 将 5×7 字形按整数倍居中放大，默认 1280×800 mode 精确得到 80×25 PTY window，不引入字体 parser 或额外 backing。空闲路径只阻塞在 ppoll/Pipe/event，不做周期刷新；输出唤醒后以 nonblocking read 合并当前 PTY burst，每批只渲染/flip 一次，单 framebuffer 同时避免额外占用一帧连续物理内存。当前是紧凑 ANSI/ASCII console，不是 compositor；图形程序直接接管 DRM master/会话的协议仍属于下一阶段，不能把 shell 可见提升为任意 2D/3D client 已兼容。
