...
use user_lib::webcore::css::{StandardCSSParser, CSSParser, StyleSheet, StyleComputer, ComputationContext};
...
fn test_style_computation() {
    ...
    if let Ok(stylesheet) = parser.parse_stylesheet(css) {
        println!("Using {} rules for style computation", stylesheet.rules.len());
