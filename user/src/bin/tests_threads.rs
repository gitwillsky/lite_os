...
use user_lib::{
    TestStats, TimeSpec, exit, getpid, gettid, nanosleep, test_section, test_subsection,
...
fn test_shared_data_access(stats: &mut TestStats) {
    ...
    unsafe {
        ...
        test_assert!(
            counter_value == thread_count * 100,
            "共享计数器值不正确: {} != {}",
            counter_value,
            thread_count * 100
        );
